...
static void InitFields(void) {
   FixedOrders = (BaseOrder *) malloc(FixedOrderCount * sizeof(BaseOrder));
   InstrZ = 0;
   ...
   AddCondition("EQ", 0x2);
   AddCondition("NE", 0x3);
   AddCondition("ULT", 0x8);
   AddCondition("ULE", 0xf);
   AddCondition("UGE", 0x9);
   AddCondition("UGT", 0xe);
   AddCondition("SLT", 0xc);
   AddCondition("SLE", 0xb);
   AddCondition("SGE", 0xd);
   AddCondition("SGT", 0xa);
   AddCondition("NET", 0x1);

   ALU2Orders = (char **)malloc(sizeof(char *) * ALU2OrderCount);
   InstrZ = 0;
   ALU2Orders[InstrZ++] = "ADD";
   ALU2Orders[InstrZ++] = "ADDC";
   ALU2Orders[InstrZ++] = "SUB";
   ALU2Orders[InstrZ++] = "SUBC";
   ALU2Orders[InstrZ++] = "CMP";
   ALU2Orders[InstrZ++] = "XOR";
   ALU2Orders[InstrZ++] = "AND";
   ALU2Orders[InstrZ++] = "OR";

   ShiftOrders = (SimpOrder *) malloc(sizeof(SimpOrder) * ShiftOrderCount);
   InstrZ = 0;
   AddShift("ASHR", 0xac);
   AddShift("ROL", 0x0c);
   AddShift("ROR", 0x2c);
   ...
   Bit2Orders = (SimpOrder *) malloc(sizeof(SimpOrder) * Bit2OrderCount);
   InstrZ = 0;
   AddBit2("BAND", 0x6a);
   AddBit2("BCMP", 0x2a);
   AddBit2("BMOV", 0x4a);
   ...
   LoopOrders = (SimpOrder *) malloc(sizeof(SimpOrder) * LoopOrderCount);
   InstrZ = 0;
   AddLoop("CMPD1", 0xa0);
   AddLoop("CMPD2", 0xb0);
   AddLoop("CMPI1", 0x80);
   ...
   DivOrders = (char **)malloc(sizeof(char *) * DivOrderCount);
   InstrZ = 0;
   DivOrders[InstrZ++] = "DIV";
   DivOrders[InstrZ++] = "DIVU";
   DivOrders[InstrZ++] = "DIVL";
   DivOrders[InstrZ++] = "DIVLU";

   BJmpOrders = (char **)malloc(sizeof(char *) * BJmpOrderCount);
   InstrZ = 0;
   BJmpOrders[InstrZ++] = "JB";
   BJmpOrders[InstrZ++] = "JNB";
   BJmpOrders[InstrZ++] = "JBC";
   BJmpOrders[InstrZ++] = "JNBS";
...
/* Pipeline-Flags weiterschalten */
   ...
   SPChanged = N_SPChanged;
   N_SPChanged = false;
   CPChanged = N_CPChanged;
   N_CPChanged = false;
         ...
         switch (AdrType) {
            case ModReg:
               ...
               switch (AdrType) {
                  case ModReg:
                     CodeLen = 2;
                     BAsmCode[0] = 0xf0 + Cond;
                     BAsmCode[1] = (HReg << 4) + AdrMode;
                     break;
                  case ModImm:
                     if (WordVal() <= 15) {
                        CodeLen = 2;
                        BAsmCode[0] = 0xe0 + Cond;
                        BAsmCode[1] = (WordVal() << 4) + HReg;
                     } else {
                        CodeLen = 4;
                        BAsmCode[0] = 0xe6 + Cond;
                  ...
                  case ModIReg:
                     CodeLen = 2;
                     BAsmCode[0] = 0xa8 + Cond;
                     BAsmCode[1] = (HReg << 4) + AdrMode;
                     break;
                  case ModPostInc:
                     CodeLen = 2;
                     BAsmCode[0] = 0x98 + Cond;
                     BAsmCode[1] = (HReg << 4) + AdrMode;
                     break;
                  case ModIndex:
                     CodeLen = 2 + AdrCnt;
                     BAsmCode[0] = 0xd4 + (Cond << 5);
                     BAsmCode[1] = (HReg << 4) + AdrMode;
                     memcpy(BAsmCode + 2, AdrVals, AdrCnt);
                     break;
                  case ModAbs:
                     CodeLen = 2 + AdrCnt;
                     BAsmCode[0] = 0xf2 + Cond;
            ...
            case ModIReg:
               ...
               switch (AdrType) {
                  case ModReg:
                     CodeLen = 2;
                     BAsmCode[0] = 0xb8 + Cond;
                     BAsmCode[1] = HReg + (AdrMode << 4);
                     break;
                  case ModIReg:
                     CodeLen = 2;
                     BAsmCode[0] = 0xc8 + Cond;
                     BAsmCode[1] = (HReg << 4) + AdrMode;
                     break;
                  case ModPostInc:
                     CodeLen = 2;
                     BAsmCode[0] = 0xe8 + Cond;
                     BAsmCode[1] = (HReg << 4) + AdrMode;
                     break;
                  case ModAbs:
                     CodeLen = 2 + AdrCnt;
                     BAsmCode[0] = 0x84 + (Cond << 5);
                     BAsmCode[1] = HReg;
                     memcpy(BAsmCode + 2, AdrVals, AdrCnt);
                     break;
               }
      ...
      if (ArgCnt != 2) WrError(1110);
         ...
         switch (AdrType) {
            case ModMReg:
               ...
               if (AdrType != ModNone) {
                  CodeLen = 4;
                  BAsmCode[0] = 0xc6 + ((AdrType == ModAbs) << 4);
                  memcpy(BAsmCode + 2, AdrVals, 2);
               }
               break;
         }
         ...
         if (ArgCnt != 2) WrError(1110);
            ...
            if (AdrType == ModReg) {
               ...
               switch (AdrType) {
                  case ModAbs:
                     CodeLen = 4;
                     BAsmCode[0] = LoopOrders[z].Code + 2;
                     BAsmCode[1] += 0xf0;
                     memcpy(BAsmCode + 2, AdrVals, 2);
                     break;
                  case ModImm:
                     if (WordVal() < 16) {
                        CodeLen = 2;
                        BAsmCode[0] = LoopOrders[z].Code;
                        BAsmCode[1] += (WordVal() << 4);
                     } else {
                        CodeLen = 4;
                        BAsmCode[0] = LoopOrders[z].Code + 6;
                        BAsmCode[1] += 0xf0;
                        memcpy(BAsmCode + 2, AdrVals, 2);
                     }
                     break;
...
/* Logik */
   ...
   if ((Memo("BCLR")) || (Memo("BSET"))) {
      if (ArgCnt != 1) WrError(1110);
         ...
         CodeLen = 2;
         BAsmCode[0] = (BOfs1 << 4) + 0x0e + Memo("BSET"); /* ANSI :-0 */
         BAsmCode[1] = BAdr1;
      }
      return;
   }
   ...
   if ((Memo("JMPS")) || (Memo("CALLS"))) {
      if ((ArgCnt != 1) && (ArgCnt != 2)) WrError(1110);
         ...
         if (ArgCnt == 1) {
            ...
            if (OK) AdrBank = EvalIntExpression(ArgStr[1], MemInt2, &OK);
            else AdrBank = 0;
         }
         if (OK) {
            CodeLen = 4;
      ...
      if (ArgCnt != 2) WrError(1110);
         ...
         switch (AdrType) {
            case ModReg:
               CodeLen = 2;
               BAsmCode[0] = 0xdc;
               BAsmCode[1] = 0x40 + (HReg << 4) + AdrMode;
            ...
            case ModImm:
               CodeLen = 4;
               BAsmCode[0] = 0xd7;
               BAsmCode[1] = 0x40 + (HReg << 4);
               if (Memo("EXTPR")) BAsmCode[1] += 0x80;
               BAsmCode[2] = (WordVal() >> 2) & 0x7f;
               BAsmCode[3] = WordVal() & 3;
               ExtCounter = HReg + 1;
      ...
      if (ArgCnt != 2) WrError(1110);
         ...
         switch (AdrType) {
            case ModReg:
               CodeLen = 2;
               BAsmCode[0] = 0xdc;
               BAsmCode[1] = 0x00 + (HReg << 4) + AdrMode;
