   ...
   switch (LastPrefix)
    {
     ...
     default: return LWordFlag && MayLW;
    }
}
...
{
   return (((strcasecmp(ArgStr[1],Name1)==0) && (strcasecmp(ArgStr[2],Name2)==0)) ||
	   ((strcasecmp(ArgStr[1],Name2)==0) && (strcasecmp(ArgStr[2],Name1)==0)));
}
   ...
   if ((Memo("EI")) || (Memo("DI")))
    {
     if (ArgCnt==0)
     ...
     else
      {
       ...
       if (OK)
	{
	 BAsmCode[0]=0xdd; BAsmCode[1]=0xf3+(Memo("EI") << 3);
	 CodeLen=3;
	}
      }
     return;
