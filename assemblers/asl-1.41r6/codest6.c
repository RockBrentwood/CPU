...
#define ASSUME62Count 1
   static ASSUMERec ASSUME62s[ASSUME62Count] = { { "ROMBASE", &WinAssume, 0, 0x3f, 0x40 } };
         ...
         do {
            ...
            if (OK) {
               ...
               if (CodeLen + strlen(s) + Flag > MaxCodeLen) {
                  ...
                  memcpy(BAsmCode + CodeLen, s, strlen(s));
                  CodeLen += strlen(s);
               ...
               if (ArgCnt != 2) WrError(1110);
                  ...
                  if (AdrType != ModNone) {
                     ...
                     switch (AdrType) {
                        case ModDir:
                           CodeLen = 2;
                           BAsmCode[0] = ALUOrders[z].Code + 0x18;
                           BAsmCode[1] = AdrVal;
                           break;
                        case ModInd:
                           CodeLen = 1;
                           BAsmCode[0] = ALUOrders[z].Code + (AdrMode << 3);
                           break;
                     }
                  }
               }
            ...
            case 'I':
               if (ArgCnt != 2) WrError(1110);
                  ...
                  if (AdrType != ModNone) {
                     ...
                     if (OK) {
                        CodeLen = 2;
                        BAsmCode[0] = ALUOrders[z].Code + 0x10;
                     }
                  }
               }
               return;
      ...
      if (ArgCnt != 1) WrError(1110);
         ...
         switch (AdrType) {
            case ModDir:
               if (IsReg(AdrVal)) {
                  CodeLen = 1;
                  BAsmCode[0] = 0x15 + ((AdrVal & 3) << 6);
                  if (Memo("DEC")) BAsmCode[0] += 8;
               } else {
                  CodeLen = 2;
                  BAsmCode[0] = 0x7f + (Memo("DEC") << 7);
                  BAsmCode[1] = AdrVal;
               }
               break;
            case ModInd:
