...
static void InitFields(void) {
   ...
   MemOrders = (FixedOrder *) malloc(sizeof(FixedOrder) * MemOrderCnt);
   InstrZ = 0;
   AddMem("LD", 0xc0);
   AddMem("ST", 0xc8);
   AddMem("AND", 0xd0);
...
static void SwitchTo_SCMP(void) {
   ...
   MakeCode = MakeCode_SCMP;
   ChkPC = ChkPC_SCMP;
   IsDef = IsDef_SCMP;
   SwitchFrom = SwitchFrom_SCMP;
   InitFields();
}
