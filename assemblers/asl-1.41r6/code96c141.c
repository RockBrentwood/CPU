...
static void AddImm(char *NName, Word NCode, bool NInSup, Byte NMinMax, Byte NMaxMax, ShortInt NDefault) {
...
static void InitFields(void) {
   ...
   MulDivOrders = (char **)malloc(sizeof(char *) * MulDivOrderCnt);
   InstrZ = 0;
   AddMulDiv("MUL");
   AddMulDiv("MULS");
   AddMulDiv("DIV");
   ...
   BitCFOrders = (ALU2Order *) malloc(sizeof(ALU2Order) * BitCFOrderCnt);
   InstrZ = 0;
   AddBitCF("ANDCF", 0);
   AddBitCF("LDCF", 3);
   AddBitCF("ORCF", 1);
...
#define RegCnt 8
   static char *Reg8Names[RegCnt] = { "A", "W", "C", "B", "E", "D", "L", "H" };
   static char *Reg16Names[RegCnt] = { "WA", "BC", "DE", "HL", "IX", "IY", "IZ", "SP" };
   static char *Reg32Names[RegCnt] = { "XWA", "XBC", "XDE", "XHL", "XIX", "XIY", "XIZ", "XSP" };
...
static bool ArgPair(const char *Val1, const char *Val2) {
   return (((strcasecmp(ArgStr[1], Val1) == 0) && (strcasecmp(ArgStr[2], Val2) == 0)) || ((strcasecmp(ArgStr[1], Val2) == 0) && (strcasecmp(ArgStr[2], Val1) == 0)));
}
...
static bool CodeMove(void) {
   ...
   if ((WMemo("POP")) || (WMemo("PUSH"))) {
      if (ArgCnt != 1) WrError(1110);
         ...
         MinOneIs0 = true;
         DecodeAdr(ArgStr[1], MModReg + MModXReg + MModMem + (WMemo("PUSH") ? MModImm : 0));
         switch (AdrType) {
            case ModReg:
               if (OpSize == 0) {
                  CodeLen = 2;
...
/* Pseudoanweisungen */
   ...
   for (z = 0; z < ImmOrderCnt; z++)
      if (Memo(ImmOrders[z].Name)) {
         if ((ArgCnt > 1) || ((ImmOrders[z].Default == -1) && (ArgCnt == 0))) WrError(1110);
            ...
            if (ArgCnt == 0) {
               AdrWord = ImmOrders[z].Default;
               OK = true;
            } else AdrWord = EvalIntExpression(ArgStr[1], Int8, &OK);
            if (!OK) ;
            else if (((Maximum) && (AdrWord > ImmOrders[z].MaxMax)) || ((!Maximum) && (AdrWord > ImmOrders[z].MinMax))) WrError(1320);
            else if (Hi(ImmOrders[z].Code) == 0) {
               CodeLen = 1;
               BAsmCode[0] = Lo(ImmOrders[z].Code) + AdrWord;
            } else {
               CodeLen = 2;
               BAsmCode[0] = Hi(ImmOrders[z].Code);
   ...
   for (z = 0; z < RegOrderCnt; z++)
      if (Memo(RegOrders[z].Name)) {
         if (ArgCnt != 1) WrError(1110);
            ...
            if (AdrType == ModNone) ;
            else if (((1 << OpSize) & RegOrders[z].OpMask) == 0) WrError(1130);
      ...
      if (WMemo(ShiftOrders[z])) {
         if ((ArgCnt != 1) && (ArgCnt != 2)) WrError(1110);
            ...
            if (ArgCnt == 1) HReg = 1;
               ...
               if (!OK) ;
               else if (FirstPassUnknown) HReg &= 0x0f;
               else if ((HReg == 0) || (HReg > 16)) {
                  WrError(1320);
                  OK = false;
               } else HReg &= 0x0f;
            }
   ...
   if ((Memo("CALL")) || (Memo("JP"))) {
      if ((ArgCnt != 1) && (ArgCnt != 2)) WrError(1110);
         ...
         if (z >= ConditionCnt) WrError(1360);
            ...
            if (AdrType != ModMem) ;
            else if ((z == DefaultCondition) && ((AdrMode == 0x41) || (AdrMode == 0x42))) {
               CodeLen = 1 + AdrCnt;
   ...
   if (Memo("CALR")) {
      if (ArgCnt != 1) WrError(1110);
         ...
         if (!OK) ;
         else if (((AdrLong < -32768) || (AdrLong > 32767)) && (!SymbolQuestionable)) WrError(1330);
         else {
            CodeLen = 3;
            BAsmCode[0] = 0x1e;
            BAsmCode[1] = Lo(AdrLong);
   ...
   if ((WMemo("INC")) || (WMemo("DEC"))) {
      if ((ArgCnt != 1) && (ArgCnt != 2)) WrError(1110);
         ...
         if (!OK) ;
         else if (FirstPassUnknown) HReg &= 7;
   ...
   if (Memo("LDX")) {
      if (ArgCnt != 2) WrError(1110);
         ...
         if (AdrType == ModNone) ;
         else if (AdrMode != 0x40) WrError(1350);
         else {
   ...
   if (((strncmp(OpPart, "MDEC", 4) == 0) || (strncmp(OpPart, "MINC", 4) == 0)) && (LChar >= '1') && (LChar <= '4')) {
      if (LChar == '3') WrError(1135);
         ...
         AdrWord = EvalIntExpression(ArgStr[1], Int16, &OK);
         if (!OK) ;
         else if (!IsPwr2(AdrWord, &HReg)) WrError(1135);
