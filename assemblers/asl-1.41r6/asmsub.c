        ...
        char *QuotPos(char *s, char Zeichen)
{
   ...
   for (i=s; *i!='\0'; i++)
    if (*i==Zeichen)
     ...
     {
      case '"': if ((Brack==0) && (AngBrack==0) && ((Flag&2)==0)) Flag^=1; break;
      case '\'':if ((Brack==0) && (AngBrack==0) && ((Flag&1)==0)) Flag^=2; break;
        ...
        char *RQuotPos(char *s, char Zeichen)
{
   ...
   for (i=s+strlen(s)-1; i>=s; i--)
    if (*i==Zeichen)
     ...
     {
      case '"': if ((Brack==0) && (AngBrack==0) && (! Quot)) Paren=! Paren; break;
      case '\'':if ((Brack==0) && (AngBrack==0) && (! Paren)) Quot=! Quot; break;
      case ')': if ((AngBrack==0) && (! Paren) && (! Quot)) Brack++; break;
      case '(': if ((AngBrack==0) && (! Paren) && (! Quot)) Brack--; break;
      case ']': if ((Brack==0) && (! Paren) && (! Quot)) AngBrack++; break;
      case '[': if ((Brack==0) && (! Paren) && (! Quot)) AngBrack--; break;
     }
        ...
        void KillSuffix(char *s)
{
   ...
   p=NULL;
   for (z=s; *z!='\0'; z++) if (*z=='\\') p=z;
   Part=(p!=NULL)?(p):(s); Part=strchr(Part,'.');
   if (Part!=NULL) *Part='\0';
}
