...
static void AddGen(char *NName, bool NMay1, bool NMay3, Byte NCode, Byte NCode3, bool NOnly, bool NSwap, bool NImm, Byte NMask1, Byte NMask3, Byte C20, Byte C21, Byte C22, Byte C23, Byte C24, Byte C25, Byte C26, Byte C27, Byte C30, Byte C31, Byte C32,
   Byte C33, Byte C34, Byte C35, Byte C36, Byte C37) {
   if (InstrZ >= GenOrderCount) exit(255);
   GenOrders[InstrZ].Name = NName;
   GenOrders[InstrZ].NameLen = strlen(NName);
   GenOrders[InstrZ].May1 = NMay1;
   GenOrders[InstrZ].May3 = NMay3;
   GenOrders[InstrZ].Code = NCode;
   GenOrders[InstrZ].Code3 = NCode3;
   GenOrders[InstrZ].OnlyMem = NOnly;
   GenOrders[InstrZ].SwapOps = NSwap;
   GenOrders[InstrZ].ImmFloat = NImm;
   GenOrders[InstrZ].ParMask = NMask1;
   GenOrders[InstrZ].Par3Mask = NMask3;
   GenOrders[InstrZ].PCodes[0] = C20;
   GenOrders[InstrZ].PCodes[1] = C21;
   GenOrders[InstrZ].PCodes[2] = C22;
   GenOrders[InstrZ].PCodes[3] = C23;
   GenOrders[InstrZ].PCodes[4] = C24;
   GenOrders[InstrZ].PCodes[5] = C25;
   GenOrders[InstrZ].PCodes[6] = C26;
   GenOrders[InstrZ].PCodes[7] = C27;
   GenOrders[InstrZ].P3Codes[0] = C30;
   GenOrders[InstrZ].P3Codes[1] = C31;
   GenOrders[InstrZ].P3Codes[2] = C32;
   GenOrders[InstrZ].P3Codes[3] = C33;
   GenOrders[InstrZ].P3Codes[4] = C34;
   GenOrders[InstrZ].P3Codes[5] = C35;
   GenOrders[InstrZ].P3Codes[6] = C36;
   GenOrders[InstrZ++].P3Codes[7] = C37;
}
...
static void InitFields(void) {
   ...
   InstrZ = 0;
   AddCondition("U", 0x00);
   AddCondition("LO", 0x01);
   AddCondition("LS", 0x02);
   AddCondition("HI", 0x03);
   AddCondition("HS", 0x04);
   AddCondition("EQ", 0x05);
   AddCondition("NE", 0x06);
   AddCondition("LT", 0x07);
   AddCondition("LE", 0x08);
   AddCondition("GT", 0x09);
   AddCondition("GE", 0x0a);
   AddCondition("Z", 0x05);
   AddCondition("NZ", 0x06);
   AddCondition("P", 0x09);
   AddCondition("N", 0x07);
   AddCondition("NN", 0x0a);
   AddCondition("NV", 0x0c);
   AddCondition("V", 0x0d);
   AddCondition("NUF", 0x0e);
   AddCondition("UF", 0x0f);
   AddCondition("NC", 0x04);
   AddCondition("C", 0x01);
   AddCondition("NLV", 0x10);
   AddCondition("LV", 0x11);
   AddCondition("NLUF", 0x12);
   AddCondition("LUF", 0x13);
   AddCondition("ZUF", 0x14);
   AddCondition("", 0x00);

   FixedOrders = (FixedOrder *) malloc(sizeof(FixedOrder) * FixedOrderCount);
   InstrZ = 0;
   AddFixed("IDLE", 0x06000000);
   AddFixed("SIGI", 0x16000000);
   AddFixed("SWI", 0x66000000);

   RotOrders = (char **)malloc(sizeof(char *) * RotOrderCount);
   InstrZ = 0;
   RotOrders[InstrZ++] = "ROL";
   RotOrders[InstrZ++] = "ROLC";
   RotOrders[InstrZ++] = "ROR";
   RotOrders[InstrZ++] = "RORC";

   StkOrders = (char **)malloc(sizeof(char *) * StkOrderCount);
   InstrZ = 0;
   StkOrders[InstrZ++] = "POP";
   StkOrders[InstrZ++] = "POPF";
   StkOrders[InstrZ++] = "PUSH";
   StkOrders[InstrZ++] = "PUSHF";

   GenOrders = (GenOrder *) malloc(sizeof(GenOrder) * GenOrderCount);
   InstrZ = 0;
/*         Name         May3      Cd3       Swap       PM1                                              PCodes3     */
/*                May1        Cd1     OMem        ImmF     PM3           PCodes1                                    */
   AddGen("ABSF", true, false, 0x00, 0xff, false, false, true, 4, 0, 0xff, 0xff, 0x04, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
   AddGen("ABSI", true, false, 0x01, 0xff, false, false, false, 8, 0, 0xff, 0xff, 0xff, 0x05, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
   AddGen("ADDC", false, true, 0x02, 0x00, false, false, false, 0, 0, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
   AddGen("ADDF", false, true, 0x03, 0x01, false, false, true, 0, 4, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x06, 0xff, 0xff, 0xff, 0xff, 0xff);
   AddGen("ADDI", false, true, 0x04, 0x02, false, false, false, 0, 8, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x07, 0xff, 0xff, 0xff, 0xff);
   AddGen("AND", false, true, 0x05, 0x03, false, false, false, 0, 8, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x08, 0xff, 0xff, 0xff, 0xff);
   AddGen("ANDN", false, true, 0x06, 0x04, false, false, false, 0, 0, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
   AddGen("ASH", false, true, 0x07, 0x05, false, false, false, 0, 8, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x09, 0xff, 0xff, 0xff, 0xff);
   AddGen("CMPF", false, true, 0x08, 0x06, false, false, true, 0, 0, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
   AddGen("CMPI", false, true, 0x09, 0x07, false, false, false, 0, 0, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
   AddGen("FIX", true, false, 0x0a, 0xff, false, false, true, 8, 0, 0xff, 0xff, 0xff, 0x0a, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
   AddGen("FLOAT", true, false, 0x0b, 0xff, false, false, false, 4, 0, 0xff, 0xff, 0x0b, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
   AddGen("LDE", false, false, 0x0d, 0xff, false, false, true, 0, 0, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
   AddGen("LDF", false, false, 0x0e, 0xff, false, false, true, 5, 0, 0x02, 0xff, 0x0c, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
   AddGen("LDFI", false, false, 0x0f, 0xff, true, false, true, 0, 0, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
   AddGen("LDI", false, false, 0x10, 0xff, false, false, false, 10, 0, 0xff, 0x03, 0xff, 0x0d, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
   AddGen("LDII", false, false, 0x11, 0xff, true, false, false, 0, 0, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
   AddGen("LDM", false, false, 0x12, 0xff, false, false, true, 0, 0, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
   AddGen("LSH", false, true, 0x13, 0x08, false, false, false, 0, 8, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x0e, 0xff, 0xff, 0xff, 0xff);
   AddGen("MPYF", false, true, 0x14, 0x09, false, false, true, 0, 52, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x0f, 0xff, 0x00, 0x01, 0xff, 0xff);
   AddGen("MPYI", false, true, 0x15, 0x0a, false, false, false, 0, 200, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x10, 0xff, 0xff, 0x02, 0x03);
   AddGen("NEGB", true, false, 0x16, 0xff, false, false, false, 0, 0, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
   AddGen("NEGF", true, false, 0x17, 0xff, false, false, true, 4, 0, 0xff, 0xff, 0x11, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
   AddGen("NEGI", true, false, 0x18, 0xff, false, false, false, 8, 0, 0xff, 0xff, 0xff, 0x12, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
   AddGen("NORM", true, false, 0x1a, 0xff, false, false, true, 0, 0, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
   AddGen("NOT", true, false, 0x1b, 0xff, false, false, false, 8, 0, 0xff, 0xff, 0xff, 0x13, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
   AddGen("OR", false, true, 0x20, 0x0b, false, false, false, 0, 8, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x14, 0xff, 0xff, 0xff, 0xff);
   AddGen("RND", true, false, 0x22, 0xff, false, false, true, 0, 0, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
   AddGen("STF", false, false, 0x28, 0xff, true, true, true, 4, 0, 0xff, 0xff, 0x00, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
   AddGen("STFI", false, false, 0x29, 0xff, true, true, true, 0, 0, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
   AddGen("STI", false, false, 0x2a, 0xff, true, true, false, 8, 0, 0xff, 0xff, 0xff, 0x01, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
   AddGen("STII", false, false, 0x2b, 0xff, true, true, false, 0, 0, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
   AddGen("SUBB", false, true, 0x2d, 0x0c, false, false, false, 0, 0, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
   AddGen("SUBC", false, false, 0x2e, 0xff, false, false, false, 0, 0, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
   AddGen("SUBF", false, true, 0x2f, 0x0d, false, false, true, 0, 4, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x15, 0xff, 0xff, 0xff, 0xff, 0xff);
   AddGen("SUBI", false, true, 0x30, 0x0e, false, false, false, 0, 8, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x16, 0xff, 0xff, 0xff, 0xff);
   AddGen("SUBRB", false, false, 0x31, 0xff, false, false, false, 0, 0, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
   AddGen("SUBRF", false, false, 0x32, 0xff, false, false, true, 0, 0, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
   AddGen("SUBRI", false, false, 0x33, 0xff, false, false, false, 0, 0, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
   AddGen("TSTB", false, true, 0x34, 0x0f, false, false, false, 0, 0, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff);
   AddGen("XOR", false, true, 0x35, 0x10, false, false, false, 0, 8, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x17, 0xff, 0xff, 0xff, 0xff);

   ParOrders = (char **)malloc(sizeof(char *) * ParOrderCount);
   InstrZ = 0;
   ParOrders[InstrZ++] = "LDF";
   ParOrders[InstrZ++] = "LDI";
   ParOrders[InstrZ++] = "STF";
   ParOrders[InstrZ++] = "STI";
   ParOrders[InstrZ++] = "ADDF3";
   ParOrders[InstrZ++] = "SUBF3";
   ParOrders[InstrZ++] = "ADDI3";
   ParOrders[InstrZ++] = "SUBI3";

   SingOrders = (SingOrder *) malloc(sizeof(SingOrder) * SingOrderCount);
   InstrZ = 0;
...
#define RegStart 0x10
   static char *Regs[RegCnt] = { "DP", "IR0", "IR1", "BK", "SP", "ST", "IE", "IF", "IOF", "RS", "RE", "RC" };
   bool Err;
...
#define ASSUME3203Count 1
   static ASSUMERec ASSUME3203s[ASSUME3203Count] = { { "DP", &DPValue, -1, 0xff, 0x100 } };
               ...
               switch (t.Typ) {
                  ...
                  case TempString:
                     for (z2 = 0; z2 < strlen(t.Contents.Ascii); z2++) {
                        ...
                        DAsmCode[CodeLen - 1] += (((LongWord) CharTransTable[(int)t.Contents.Ascii[z2]])) << (8 * (3 - (z2 & 3)));
                     }
                     break;
                  case TempNone:
                     OK = false;
...
/* Arithmetik/Logik */
   ...
   for (z = 0; z < GenOrderCount; z++)
      ...
      /* Argumente parsen */
         if (Is3) {
            if (Memo("TSTB3")) {
               ...
               CurrDestMode = AdrMode;
               CurrDestPart = AdrPart;
            }
            ...
            CurrSrc2Mode = AdrMode;
            CurrSrc2Part = AdrPart;
            ...
            CurrSrc1Mode = AdrMode;
            CurrSrc1Part = AdrPart;
         } else { /* ! Is3 */
            ...
            switch (AdrMode) {
               case ModReg:
                  CurrDestMode = AdrMode;
                  CurrDestPart = AdrPart;
                  CurrSrc2Mode = CurrSrc1Mode;
                  CurrSrc2Part = CurrSrc1Part;
                  break;
               case ModInd:
      ...
      /* ...sequentiell */
            ...
            PrevSrc1Mode = CurrSrc1Mode;
            PrevSrc1Part = CurrSrc1Part;
            PrevSrc2Mode = CurrSrc2Mode;
            ...
            if (Is3)
               ...
               DAsmCode[0] = 0x00000000 + (((LongWord) GenOrders[z].Code) << 23)
                  + (((LongWord) CurrSrc1Mode) << 21) + CurrSrc1Part + (((LongWord) CurrDestPart) << 16);
            CodeLen = 1;
            NextPar = true;
         }
         return;
         ...
         if (Memo(Conditions[z].Name)) {
            if (ArgCnt != 2) WrError(1110);
               ...
               if (DecodeReg(ArgStr[2], &HReg)) {
                  DAsmCode[0] = 0x6c000000 + (((LongWord) Conditions[z].Code) << 16)
