...
;Note: go/done bit should not be set in the same instruction
;which turns on the a/d.
loop
	btfss   ADCON0,2        ;a/d done?
	goto    update          ;yes then update new value.
	goto    loop            ;no then keep checking
;
service_int
	return                  ;why not use retfie?
;
PAGE
;
...
InitializeAD
	...
	movlw   B'00011001'     ;select:fosc/2,ch3..
