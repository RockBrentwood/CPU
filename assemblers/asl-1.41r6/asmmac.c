...
static void EnterDefine(char *Name, char *Definition) {
   ...
   if (!CaseSensitive) NLS_UpString(Neu->TransFrom);
...
static bool AddMacro_AddNode(PMacroNode * Node, PMacroRec Neu, LongInt DefSect, bool Protest) {
   bool Grown;
   PMacroNode p1, p2;
   bool Result;
