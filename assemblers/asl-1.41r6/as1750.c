// Mil-Std-1750 Assembler and Linker
// 1750 instruction assembly
// Copyright (C) Daimler-Benz Aerospace AG, 1994-1997
// Author:
//	Oliver M. Kellogg, Dornier Satellite Systems,
...
#ifdef AS1750
   fprintf(stderr, "%s line%5d: %s\n", nopath(file[curr_file].name), file[curr_file].line_number, output_line);
#else /* ASL */
...
/* determine if Intel format */
   ...
   if (intel || (c_lang = (*s == '0' && upcase(*(s + 1)) == 'X'))
...
void add_word(ushort word) {
   ...
   if (obj->data == (ushort *) 0) {
      ...
      obj->line = (struct linelist **)malloc(obj->n_allocated * sizeof(struct linelist *));
   } else if (obj->n_used == obj->n_allocated) {
      obj->n_allocated *= 2;
      obj->data = (ushort *) realloc(obj->data, obj->n_allocated * sizeof(ushort));
      obj->line = (struct linelist **)realloc(obj->line, obj->n_allocated * sizeof(struct linelist *));
   }
...
void add_reloc(symbol_t sym) { /* auxiliary to parse_addr() */
   struct reloc *rel;

   if (relblk.data == (struct reloc *)0) {
      relblk.n_allocated = 256;
      relblk.data = (struct reloc *)malloc(relblk.n_allocated * sizeof(struct reloc));
   } else if (relblk.n_used == relblk.n_allocated) {
      relblk.n_allocated *= 2;
      relblk.data = (struct reloc *)realloc(relblk.data, relblk.n_allocated * sizeof(struct reloc));
   }
   if (relblk.data == (struct reloc *)0)
         ...
         /* subtraction */
         {
            if (sym->is_constant) {
               ...
               if (sym1->is_constant) {
                  ...
                  if (objblk[sym->frag_index].section != objblk[sym1->frag_index].section)
                     return error("cannot subtract relocatable symbols from different fragments");
...
static char *get_sym_num(char *s, int *outnum) {
   bool okay;
...
static status parse_addr(char *s) {
   int value;
   bool okay;
...
static ushort get_num_bounded(char *s, int lowlim, int highlim) {
   int num;
...
static ushort get_regnum(char *s) {
   ushort regnum;
   *s = toupper(*s);
   if (*s != 'R')
...
static status check_indexreg(void) {
   if (n_args > 2) {
      ushort rx;
...
static ushort as_mem(ushort oc) {
   ushort ra;
   if (n_args < 2)
...
static ushort as_addr(ushort oc) { /* LST and LSTI */
   if (n_args < 1)
      return error("insufficient number of operands");
   add_word(oc);
   if (parse_addr(arg[0]))
...
static ushort as_xmem(ushort oc) { /* MIL-STD-1750B extended mem. addr. */
   ushort ra;
...
static ushort as_im_0_15(ushort oc) { /* for STC, LM, TB,SB,RB,TSB */
   ushort ra;
   if (n_args < 2)
      return error("insufficient number of operands");
...
static ushort as_im_1_16(ushort oc) {
   ushort ra;
   if (n_args < 2)
      return error("insufficient number of operands");
...
static ushort as_im_ocx(ushort oc) {
   ushort ra;
   if (n_args != 2)
      return error("incorrect number of operands");
   if ((ra = get_regnum(arg[0])) == ERROR)
...
static ushort as_icr(ushort oc) {
#ifdef AS1750
   struct objblock *obj = &objblk[curr_frag];
   int last = obj->n_used;
#endif
...
/* If symbol relocation, then set the relocation type to Byte_Reloc */
...
#else /* ASL */
   {
      ...
      if (diff < -128L || diff > 127L)
         return error("address distance too large in Instruction Counter Relative operation");
...
static ushort as_r_imm(ushort oc) { /* for shifts with immediate shiftcount */
   ushort rb, n;
   if (n_args != 2)
      return error("incorrect number of operands");
...
static ushort as_imm_r(ushort oc) { /* for test/set/reset-bit in reg. */
   ushort n, rb;
   if (n_args != 2)
      return error("incorrect number of operands");
...
static ushort as_jump(ushort oc) {
...
static ushort as_s(ushort oc) { /* For the moment, BEX only. */
   ushort lsnibble;
   if (n_args != 1)
      return error("incorrect number of operands");
...
static ushort as_sr(ushort oc) { /* XBR and URS. */
   ushort hlp;
   if (n_args != 1)
      return error("incorrect number of operands");
...
static ushort as_xio(ushort oc) {
   ushort ra;
   int cmdfld = -1;
   static const struct {
      char *mnem;
...
static ushort as_none(ushort oc) {
   add_word(oc);
   return 1;
}
...
ushort as1750(char *operation, int n_operands, char *operand[]) {
