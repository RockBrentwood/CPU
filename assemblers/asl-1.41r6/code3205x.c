...
static Word eval_ar_expression(char *asc, bool *ok) {
   *ok = true;
   ...
   if ((toupper(asc[0]) == 'A') && (toupper(asc[1]) == 'R') && (asc[2] >= '0') && (asc[2] <= '7') && (asc[3] <= '\0'))
      return asc[2] - '0';
...
static Word decode_cond(Integer argp) {
   ...
   while (argp <= ArgCnt) {
      for (cndp = cond_tab; cndp->name && strcasecmp(cndp->name, ArgStr[argp]); cndp++);
      if (!cndp->name) {
         WrError(1360);
         return ret;
      }
...
typedef void (*tcallback)(bool *, Integer *, LongInt);
      ...
      for (z = 1; (z <= ArgCnt) && ok; z++) {
         ...
         switch (t.Typ) {
            ...
            case TempString:
               ...
               while (*cp) {
                  if (z2 & 1)
                     WAsmCode[CodeLen++] |= (CharTransTable[*cp++]
                        << 8);
                  else
                     WAsmCode[CodeLen] = CharTransTable[*cp++];
                  z2++;
...
/* Qxx */
   ...
   if ((OpPart[0] == 'Q') && (OpPart[1] >= '0') && (OpPart[1] <= '9') && (OpPart[2] >= '0') && (OpPart[2] <= '9') && (OpPart[3] == '\0')) {
...
/* LQxx */
   ...
   if ((OpPart[0] == 'L') && (OpPart[1] == 'Q') && (OpPart[2] >= '0') && (OpPart[2] <= '9') && (OpPart[3] >= '0') && (OpPart[3] <= '9') && (OpPart[4] == '\0')) {
...
/* Immediate */
   ...
   if (Memo("MPY") && (ArgCnt == 1) && (ArgStr[1][0] == '#')) {
      ...
      if (!ok)
         return;
      if (FirstPassUnknown || (adr_long < -4096) || (adr_long > 4095)) {
         CodeLen = 2;
         WAsmCode[0] = 0xbe80;
         WAsmCode[1] = adr_long;
      } else {
   ...
   if ((Memo("AND") || Memo("OR") || Memo("XOR")) && (ArgCnt >= 1) && (ArgCnt <= 2) && (ArgStr[1][0] == '#')) {
      WAsmCode[1] = EvalIntExpression((ArgStr[1]) + 1, Int16, &ok);
      adr_word = 0;
      adr_ok = true;
      if (ArgCnt >= 2)
         adr_word = EvalIntExpression(ArgStr[2], UInt5, &adr_ok);
      if ((!ok) || (!adr_ok))
         return;
...
/* Adresse, spezial */
   ...
   if (Memo("LST") || Memo("SST")) {
      ...
      if ((!ok) || (!adr_ok))
         return;
      CodeLen = 1;
      WAsmCode[0] = ((Memo("SST")) ? 0x8e00 : 0x0e00) | ((adr_word & 1) << 8) | adr_mode;
      return;
   }
...
/* Spruenge */
   ...
   if (Memo("XC")) {
      ...
      if (!ok)
         return;
      CodeLen = 1;
      WAsmCode[0] = ((adr_mode == 2) ? 0xf400 : 0xe400) | decode_cond(2);
      return;
   }
...
/* spezialbefehle */
   ...
   if (Memo("SACL") || Memo("SACH")) {
      ...
      if ((!adr_ok) || (!ok))
         return;
      CodeLen = 1;
      WAsmCode[0] = ((Memo("SACH")) ? 0x9800 : 0x9000) | adr_mode | ((adr_word & 7) << 8);
      return;
   }
...
/* Auxregisterbefehle */
   ...
   if (Memo("ADRK") || Memo("SBRK")) {
      ...
      if (!ok)
         return;
      CodeLen = 1;
      WAsmCode[0] = ((Memo("SBRK")) ? 0x7c00 : 0x7800) | (adr_word & 0xff);
      return;
   }
   ...
   if (Memo("LAR")) {
      ...
      if (ArgStr[2][0] == '#') {
         if (ArgCnt > 2)
            WrError(1110);
         adr_long = EvalIntExpression((ArgStr[2]) + 1, Int16, &adr_ok) & 0xffff;
         if ((!adr_ok) || (!ok))
            return;
         if (FirstPassUnknown || (adr_long > 255)) {
            CodeLen = 2;
         ...
         CodeLen = 1;
         WAsmCode[0] = 0xb000 | ((adr_word & 7) << 8) | (adr_long & 0xff);
         return;
      }
      decode_adr(ArgStr[2], 3, false);
      if ((!adr_ok) || (!ok))
   ...
   if (Memo("ADD") || Memo("SUB")) {
      ...
      if (ArgStr[1][0] == '#') {
         ...
         if ((ArgCnt > 1) || FirstPassUnknown || (adr_long < 0)
            || (adr_long > 255)) {
            if (ArgCnt > 1)
               adr_word = EvalIntExpression(ArgStr[2], UInt4, &adr_ok);
            if ((!adr_ok) || (!ok))
               return;
            CodeLen = 2;
            WAsmCode[0] = ((Memo("SUB")) ? 0xbfa0 : 0xbf90) | (adr_word & 0xf);
            WAsmCode[1] = adr_long;
            return;
         }
         if (!ok)
            return;
         CodeLen = 1;
         WAsmCode[0] = ((Memo("SUB")) ? 0xba00 : 0xb800) | (adr_long & 0xff);
         return;
      }
      adr_word = 0;
      ok = true;
      ...
      CodeLen = 1;
      if (adr_word >= 16) {
         WAsmCode[0] = ((Memo("SUB")) ? 0x6500 : 0x6100) | adr_mode;
         return;
      }
      WAsmCode[0] = ((Memo("SUB")) ? 0x3000 : 0x2000) | ((adr_word & 0xf) << 8) | adr_mode;
      return;
   }
