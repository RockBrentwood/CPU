...
/* Ein/Ausgabe */
   ...
   if ((Memo("IN")) || (Memo("OUT"))) {
      ...
      ChkSpace(SegIO);
      CodeLen = 1;
      WAsmCode[0] = ((Memo("OUT")) ? 0xe000 : 0x8000) | adr_mode | (adr_word << 8);
      return;
   }
...
/* konstantes Argument */
   ...
   for (io = cmd_imm_order; io->name; io++) {
      if (Memo(io->name)) {
         if ((ArgCnt < 1) || (ArgCnt > 2) || ((ArgCnt == 2) && (io->mask != 0xffff))) {
            WrError(1110);
            return;
         }
...
/* mit Hilfsregistern */
   ...
   if ((Memo("LAR")) || (Memo("SAR"))) {
      ...
      if (!adr_ok)
         return;
      CodeLen = 1;
      WAsmCode[0] = ((Memo("SAR")) ? 0x7000 : 0x3000) | adr_mode | (adr_word << 8);
      return;
   }
...
static bool check_pc_3202x(void) {
   switch (ActPC) {
      case SegCode:
      case SegData:
...
static void switch_to_3202x(void) {
   ...
   ChkPC = check_pc_3202x;
   IsDef = is_def_3202x;
   SwitchFrom = switch_from_3202x;
}
