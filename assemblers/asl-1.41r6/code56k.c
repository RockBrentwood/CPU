...
#define RegCount 12
   ...
   return false;
	...
	static bool DecodeALUReg(char *Asc, LongInt *Erg,
                                    bool MayX, bool MayY, bool MayAcc)
{
   bool Result=false;
	...
	Result=true; (*Erg)-=6;
     ...
     case 15:
      if (MayAcc)
       {
	Result=true;
	if ((MayX) || (MayY)) (*Erg)-=12; else (*Erg)-=14;
       }
      break;
    }
	...
	static bool DecodeControlReg(char *Asc, LongInt *Erg)
{
   ...
   return Result;
	...
	static bool DecodeOpPair(char *Left, char *Right, Byte WorkSeg,
       ...
       else if (DecodeGeneralReg(Left,&Reg1))
	{
	 ...
	 if (AdrType!=ModNone)
	  if ((AdrVal>0xffff) || (AdrVal<0xffc0)) WrError(1315);
	   ...
	   {
	    CodeLen=1;
	    DAsmCode[0] = 0x08c000 + ((AdrSeg==SegYData) << 16) +
			  (AdrVal & 0x3f) + (Reg1 << 8);
	   }
	}
       else if (DecodeGeneralReg(Right,&Reg2))
	{
	 ...
	 if (AdrType!=ModNone)
	  if ((AdrVal>0xffff) || (AdrVal<0xffc0)) WrError(1315);
	   ...
	   {
	    CodeLen=1;
	    DAsmCode[0] = 0x084000 + ((AdrSeg==SegYData) << 16) +
			  (AdrVal & 0x3f) + (Reg2 << 8);
	   }
	}
       else
