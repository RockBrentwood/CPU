        ...
        void ConstStringVal(char *Asc_O, char *Erg, bool *OK)
{
   ...
   *OK=false;
   ...
   while (*Asc!='\0')
    {
     z=strchr(Asc,'\\'); if (z==NULL) z=Asc+strlen(Asc);
     Save=(*z); *z='\0'; if (strchr(Asc,'"')!=NULL) return;
       ...
       switch (toupper(Asc[1]))
        {
         ...
         case '5': case '6': case '7': case '8': case '9':
          ...
          if (! OK2) return; tmp[l++]='\0';
          break;
         case '{':
          z=QuotPos(Asc,'}'); if (z==NULL) return;
          FirstPassUnknown=false;
   ...
   for (zp=Asc; *zp!='\0'; zp++)
    {
     switch (*zp)
      {
       case '(': if (! (InHyp || InQuot)) LKlamm++; break;
       case ')': if (! (InHyp || InQuot)) RKlamm++; break;
       case '{': if (! (InHyp || InQuot)) WKlamm++; break;
       case '}': if (! (InHyp || InQuot)) WKlamm--; break;
       case '"': if (! InHyp) InQuot=! InQuot; break;
       case '\'':if (! InQuot) InHyp=! InHyp; break;
       default:
       ...
       for (z1=1; z1<=ValFunc->ArguCnt; z1++)
        {
         ...
         KlPos=QuotPos(Asc,','); if (KlPos!=NULL) *KlPos='\0';
         EvalExpression(Asc,&LVal);
         if (KlPos==NULL) Asc[0]='\0'; else strcpy(Asc,KlPos+1);
         switch (LVal.Typ)
          {
           case TempInt:
        ...
        bool EnterTreeNode(SymbolPtr *Node, SymbolPtr Neu, bool MayChange, bool DoCross)
{
   ...
   if (*Node==NULL)
    {
     (*Node)=Neu;
     (*Node)->Balance=0; (*Node)->Left=NULL; (*Node)->Right=NULL;
     ...
     case 0:
      if (((*Node)->Defined) && (! MayChange))
      ...
      else
       {
        ...
        Neu->RefList=(*Node)->RefList; (*Node)->RefList=NULL;
...
{
   *Lauf=(*Root); *Prev=NULL; *RRoot=Root;
        ...
        static void AddReference(SymbolPtr Node)
{
   ...
   Neu->LineNum=CurrLine; Neu->OccNum=1; Neu->Next=NULL;
   ...
   /* ansonsten an Kettenende anhaengen */
   ...
   else
    {
     ...
     Lauf->Next=Neu;
        ...
        static bool FindNode_FNode(char *Name, TempType SearchType,
...
/* if (FSpec(SectionStack->GlobSyms)) return; */
   ...
   return FindNode_Result;
        ...
        static bool FindLocNode_FNode(char *Name, TempType SearchType,
        ...
        static SymbolPtr FindLocNode(char *Name_O, TempType SearchType)
{
   ...
   strmaxcpy(Name,Name_O,255); if (! CaseSensitive) NLS_UpString(Name);
   ...
   if ((LStack==NULL) || (strcmp(LStack->Name,StackName)>0))
    {
     ...
     NStack->Next=LStack;
     if (PStack==NULL) FirstStack=NStack; else PStack->Next=NStack;
     LStack=NStack;
    }
        ...
        bool PopSymbol(char *SymName_O, char *StackName_O)
{
   ...
   if ((LStack->Contents=Elem->Next)==NULL)
    {
     if (PStack==NULL) FirstStack=LStack->Next; else PStack->Next=LStack->Next;
     free(LStack->Name);
     free(LStack);
    }
   free(Elem);
        ...
        LongInt GetSectionHandle(char *SName_O, bool AddEmpt, LongInt Parent)
{
   ...
   strmaxcpy(SName,SName_O,255); if (! CaseSensitive) NLS_UpString(SName);

   Lauf=FirstSection; Prev=NULL; z=0;
    ...
    if (AddEmpt)
     {
      ...
      if (Prev==NULL) FirstSection=Lauf; else Prev->Next=Lauf;
     }
    else z=(-2);
   return z;
}
