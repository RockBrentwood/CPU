...
static void InitFields(void) {
   ...
   ALU3Orders = (FixedOrder *) malloc(sizeof(FixedOrder) * ALU3OrderCnt);
   InstrZ = 0;
   AddALU3("ADD", 0x01);
   AddALU3("AND", 0x00);
   AddALU3("MUL", 0x83); /* ** */
   ...
   ALU1Orders = (FixedOrder *) malloc(sizeof(FixedOrder) * ALU1OrderCnt);
   InstrZ = 0;
   AddALU1("CLR", 0x01);
   AddALU1("DEC", 0x05);
   AddALU1("EXT", 0x06);
   ...
   MacOrders = (MacOrder *) malloc(sizeof(MacOrder) * MacOrderCnt);
   InstrZ = 0;
   AddMac("MAC", 0x00, false);
   AddMac("SMAC", 0x01, false);
   AddMac("MACR", 0x04, true);
   AddMac("SMACR", 0x05, true);
   AddMac("MACZ", 0x08, false);
   AddMac("SMACZ", 0x09, false);
   AddMac("MACRZ", 0x0c, true);
   AddMac("SMACRZ", 0x0d, true);

   RptOrders = (FixedOrder *) malloc(sizeof(FixedOrder) * RptOrderCnt);
   InstrZ = 0;
   AddRpt("RPT", 0x00);
   AddRpt("RPTNST", 0x10);
   AddRpt("RPTNH", 0x11);
   ...
   if (p != NULL) {
      ...
      if ((p2 > Asc + l - 1) || (p2 < Asc + l - 2)) WrError(1350);
         ...
         *p2 = '\0';
         ...
         if (!OK) ;
         else if (!ChkWork(&BReg)) WrError(1320);
         else {
            Reg = Lo(BReg);
            ChkSFR(Reg);
      ...
      if (!OK) ;
      else if ((AdrWord & OMask) != 0) WrError(1325);
      else {
         BReg = AdrWord & 0xffff;
...
/* Arithmetik */
   ...
   for (z = 0; z < ALU3OrderCnt; z++)
      if (BMemo(ALU3Orders[z].Name)) {
         if ((ArgCnt != 2) && (ArgCnt != 3)) WrError(1110);
            ...
            Start = 0;
            Special = (strncmp(ALU3Orders[z].Name, "MUL", 3) == 0);
   ...
   for (z = 0; z < ALU2OrderCnt; z++)
      if (BMemo(ALU2Orders[z].Name)) {
         if (ArgCnt != 2) WrError(1110);
            ...
            Start = 0;
            Special = (strncmp(OpPart, "DIV", 3) == 0);
      ...
      if (LMemo(ShiftOrders[z])) {
         if (ArgCnt != 2) WrError(1110);
            ...
            if (AdrType != ModNone) {
               ...
               if (AdrType == ModNone) ;
               else if ((AdrType == ModImm) && (AdrVals[0] > 15)) WrError(1320);
   ...
   if ((BMemo("ELD")) || (BMemo("EST"))) {
      if (ArgCnt != 2) WrError(1110);
         ...
         if (AdrType != ModMem) ;
         else if ((AdrMode == 2) && (Odd(AdrVals[0]))) WrError(1350); /* kein Autoincrement */
         else {
      ...
      if (Memo(MacOrders[z].Name)) {
         if ((ArgCnt != 1) && (ArgCnt != 2)) WrError(1110);
            ...
            if (AdrType != ModNone) {
               ...
               if (AdrType != ModNone) {
                  BAsmCode[HReg] = MacOrders[z].Code;
                  CodeLen = 1 + HReg;
               }
            }
         }
   ...
   if ((Memo("MVAC")) || (Memo("MSAC"))) {
      if (ArgCnt != 2) WrError(1110);
         ...
         if (AdrType == ModDir) {
            BAsmCode[0] = 0x0d;
            BAsmCode[2] = AdrVals[0] + 1 + (Memo("MSAC") << 1);
            OpSize = 0;
            DecodeAdr(ArgStr[2], MModImm + MModDir, false);
            BAsmCode[1] = AdrVals[0];
            switch (AdrType) {
               case ModImm:
                  if (AdrVals[0] > 31) WrError(1320);
                  else CodeLen = 3;
                  break;
               case ModDir:
                  if (AdrVals[0] < 32) WrError(1315);
                  else CodeLen = 3;
            }
         }
      }
   ...
   for (z = 0; z < RptOrderCnt; z++)
      if (Memo(RptOrders[z].Name)) {
         if (ArgCnt != 1) WrError(1110);
            ...
            if (AdrType == ModNone) ;
            else if (AdrMode == 3) WrError(1350);
            else {
...
/* Spruenge */
   ...
   if ((Memo("BR")) || (Memo("LJMP")) || (Memo("SJMP"))) {
      ...
      if (ArgCnt != 1) WrError(1110);
         ...
         if (AdrType == ModNone) ;
         else if ((AdrMode != 2) || ((AdrVals[0] & 1) == 1)) WrError(1350);
         else {
            CodeLen = 2;
            BAsmCode[0] = 0xe3;
            BAsmCode[1] = AdrVals[0];
   ...
   if ((Memo("EJMP")) || (Memo("EBR"))) {
      ...
      if (ArgCnt != 1) WrError(1110);
         ...
         DecodeAdr(ArgStr[1], MModMem, false);
         if (AdrType != ModMem) ;
         else if (AdrMode != 2) WrError(1350);
