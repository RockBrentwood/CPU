...
static void InitFields(void) {
   ...
   InstrZ = 0;
   Gen1Orders = (FixedOrder *) malloc(sizeof(FixedOrder) * Gen1OrderCnt);
   AddGen1("ABS", 0x76f0);
   AddGen1("ADCF", 0x76e0);
   AddGen1("NEG", 0x7450);
   AddGen1("ROLC", 0x76a0);
   ...
   InstrZ = 0;
   Gen2Orders = (Gen2Order *) malloc(sizeof(Gen2Order) * Gen2OrderCnt);
   ...
   InstrZ = 0;
   DivOrders = (Gen2Order *) malloc(sizeof(Gen2Order) * DivOrderCnt);
   ...
   InstrZ = 0;
   BCDOrders = (char **)malloc(sizeof(char *) * BCDOrderCnt);
   AddBCD("DADD");
   AddBCD("DSUB");
   AddBCD("DADC");
   AddBCD("DSBB");
   ...
   InstrZ = 0;
   DirOrders = (char **)malloc(sizeof(char *) * DirOrderCnt);
   AddDir("MOVLL");
   AddDir("MOVHL");
   AddDir("MOVLH");
   AddDir("MOVHH");
...
/* Datenregister 8 Bit */
   ...
   if ((strlen(Asc) == 3) && (toupper(*Asc) == 'R') && (Asc[1] >= '0') && (Asc[1] <= '1') && ((toupper(Asc[2]) == 'L') || (toupper(Asc[2]) == 'H'))) {
      AdrType = ModGen;
   ...
   if ((p != NULL) && (Asc[strlen(Asc) - 1] == ']')) {
      ...
      if ((strcasecmp(RegPart, "A0") == 0) || (strcasecmp(RegPart, "A1") == 0)) {
         *p = '\0';
         ...
         if (!OK) ;
         else if ((DispAcc == 0) && ((Mask & MModGen) != 0)) {
            AdrType = ModGen;
         ...
         *p = '\0';
         DispAcc = EvalIntExpression(Asc, SInt8, &OK);
         if (!OK) ;
         else if (DispAcc != 0) WrError(1320);
...
static bool DecodeReg(char *Asc, Byte * Erg) {
   ...
   else if ((strlen(Asc) == 2) && (toupper(*Asc) == 'A') && (Asc[1] >= '0') && (Asc[1] <= '1')) *Erg = Asc[1] - '0' + 4;
   else if ((strlen(Asc) == 2) && (toupper(*Asc) == 'R') && (Asc[1] >= '0') && (Asc[1] <= '3')) *Erg = Asc[1] - '0';
   else return false;
   return true;
}
...
static bool CodeData(void) {
      ...
      if (ArgCnt != 2) WrError(1110);
         ...
         if (strcasecmp(ArgStr[2], "PC") == 0)
            ...
            if (AdrType == ModImm) {
               ...
               BAsmCode[0] = 0x7a + z;
               BAsmCode[1] = 0x80 + (SMode << 4) + AdrMode;
               memcpy(BAsmCode + 2, AdrVals, AdrCnt);
               CodeLen = 2 + AdrCnt;
            }
         }
   ...
   if ((Memo("LDCTX")) || (Memo("STCTX"))) {
      if (ArgCnt != 2) WrError(1110);
         ...
         if (AdrType != ModGen) ;
         else if (AdrMode != 15) WrError(1350);
         else {
            memcpy(BAsmCode + 2, AdrVals, AdrCnt);
   ...
   if ((Memo("LDE")) || (Memo("STE"))) {
      if (ArgCnt != 2) WrError(1110);
         ...
         if (AdrType == ModNone) ;
            ...
            CopyAdr();
            DecodeAdr(ArgStr[2], MModAbs20 + MModDisp20 + MModIReg32);
            if (AdrType != ModNone) {
               BAsmCode[0] = 0x74 + OpSize;
   ...
   if (Memo("MOVA")) {
      if (ArgCnt != 2) WrError(1110);
         ...
         DecodeAdr(ArgStr[1], MModGen);
         if (AdrType == ModNone) ;
         else if (AdrMode < 8) WrError(1350);
         else {
            CopyAdr();
            DecodeAdr(ArgStr[2], MModGen);
            if (AdrType == ModNone) ;
            else if (AdrMode > 5) WrError(1350);
            else {
               BAsmCode[0] = 0xeb;
               BAsmCode[1] = (AdrMode << 4) + AdrMode2;
               memcpy(BAsmCode + 2, AdrVals2, AdrCnt2);
      ...
      if (Memo(DirOrders[z])) {
         if (ArgCnt != 2) WrError(1110);
            ...
            if (!OK) WrError(1350);
               ...
               if (AdrType == ModNone) ;
               else if (((AdrMode & 14) == 4) || ((AdrMode == 0) && (Num1 == 1))) WrError(1350);
               else {
                  BAsmCode[0] = 0x7c;
                  BAsmCode[1] = (Num1 << 7) + (z << 4) + AdrMode;
                  memcpy(BAsmCode + 2, AdrVals, AdrCnt);
                  CodeLen = 2 + AdrCnt;
               }
            }
   ...
   for (z = 0; z < StringOrderCnt; z++)
      if (Memo(StringOrders[z].Name)) {
         ...
         if (ArgCnt != 0) WrError(1110);
            ...
            BAsmCode[0] = Lo(StringOrders[z].Code) + OpSize;
            CodeLen = 1;
         } else {
            BAsmCode[0] = Hi(StringOrders[z].Code) + OpSize;
            BAsmCode[1] = Lo(StringOrders[z].Code);
            CodeLen = 2;
         }
         return;
      }
...
/* Arithmetik */
         ...
         if (AdrType != ModNone) {
            ...
            if (AdrType == ModNone) ;
               ...
               if (FormatCode == 0) {
                  ...
                  else if ((OpSize == 0) && (IsShort(AdrMode, &SMode)) && (AdrMode2 <= 1) && ((AdrMode > 1) || (Odd(AdrMode ^ AdrMode2)))) FormatCode = 3;
                  else FormatCode = 1;
               }
               switch (FormatCode) {
                  case 1:
   ...
   if (Memo("NOT")) {
      ...
      if (AdrType == ModNone) ;
            ...
            if (FormatCode != 0) ;
            else if ((OpSize == 0) && (IsShort(AdrMode, &SMode))) FormatCode = 2;
            else FormatCode = 1;
            switch (FormatCode) {
               case 1:
                  BAsmCode[0] = 0x74 + OpSize;
      ...
      if (ArgCnt != 2) WrError(1110);
         ...
         if (AdrType != ModNone) {
            ...
            if (AdrType == ModNone) ;
               ...
               else if ((AdrMode2 <= 1) && (IsShort(AdrMode, &SMode)) && ((AdrMode > 1) || Odd(AdrMode ^ AdrMode2))) FormatCode = 2;
               else FormatCode = 1;
               switch (FormatCode) {
                  case 1:
...
/* Bitoperationen */
   ...
   for (z = 0; z < BitOrderCnt; z++)
      if (Memo(BitOrders[z].Name)) {
         ...
         if (CheckFormat(MayShort ? "GS" : "G") && DecodeBitAdr(FormatCode != 1 && MayShort)) {
            if (AdrMode >= 16) {
...
/* Spruenge */
   ...
   if ((Memo("JMPS")) || (Memo("JSRS"))) {
      if (ArgCnt != 1) WrError(1110);
         ...
         if (AdrType == ModNone) ;
         else if (AdrVals[0] < 18) WrError(1315);
         else {
      ...
      for (z = 0; z < ConditionCnt; z++)
         if (strcmp(Conditions[z].Name, OpPart + 1) == 0) {
            ...
            if (ArgCnt != 1) WrError(1110);
               ...
               if (!OK) ;
               else if ((!SymbolQuestionable) && ((AdrLong > 127) || (AdrLong < -128))) WrError(1370);
               else if (Conditions[z].Code >= 8) {
                  BAsmCode[0] = 0x7d;
                  BAsmCode[1] = 0xc0 + Conditions[z].Code;
                  BAsmCode[2] = AdrLong & 0xff;
                  CodeLen = 3;
               } else {
                  BAsmCode[0] = 0x68 + Conditions[z].Code;
                  BAsmCode[1] = AdrLong & 0xff;
                  CodeLen = 2;
               }
            }
      ...
      if (ArgCnt != 3) WrError(1110);
         ...
         if (AdrType == ModNone) ;
            ...
            if (ChkRange(Num1, -8, 7)) {
               ...
               if (!OK) ;
               else if (((AdrLong < -128) || (AdrLong > 127)) && (!SymbolQuestionable)) WrError(1370);
               else {
   ...
   if (Memo("LDINTB")) {
      if (ArgCnt != 1) WrError(1110);
         ...
         if (OK) {
            ...
            BAsmCode[4] = 0xeb;
            BAsmCode[5] = 0x10;
            BAsmCode[6] = (AdrLong >> 8) & 0xff;
            BAsmCode[7] = AdrLong & 0xff;
            CodeLen = 8;
         }
      }
      return;
