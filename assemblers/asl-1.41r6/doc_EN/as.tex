%% Hello altogether,
%%
%% this is the first BETA release of the english AS manual.  I haven't
...
{\em IBM, PPC403GA, and PowerPC} are trademarks of IBM Corporation.

{\em UNIX} is a registered trademark of X/Open Company.

{\em Windows} is a registered trademark of Microsoft Corporation.
...
Digital Unix, Linux, and OS/2 operating systems running on AMD 5x86 and
...
\item{by Surface Mail: \begin{description}
               \item{Alfred Arnold}
               \item{Br\"oltalstr. 5a}
               \item{D-53773 Hennef (Sieg)}
               \item{Germany}
               \end{description}}
\item{by E-Mail: \tty{a.arnold@kfa-juelich.de}}
\end{itemize}
...
bxy mail.  Only requests containing floppies (2 pieces 1.44 Mbytes, for
...
\begin{itemize}
\item{Motorola 68000..68030,683xx incl. coprocessor and MMU}
...
due to user requests.  So, the version number is not only marketing, but
expresses really that this is no 1.0 version with beta-version bugs like
MS-DOS 6.0 (which does not reduce the importance of what has been said in
part \ref{SectLicense}).
...
large and complex programs, there is a DOS version that runs in protected
mode via a DOS extender and can therefore make use of the whole extended
...
The DOS version \marginpar{{\em DOS}} will run principally run on any
...
The DOS package \marginpar{{\em DOS}} released by me contains the files
listed in tables \ref{TabPackageList1} and \ref{TabPackageList2}, which can
roughly be divided in program files, documentation, includes, and test
programs:
...
AS2MSG.EXE           & error message filter AS $\longrightarrow$ Borland-Pascal \\
\hline
...
STDDEF16.INC         & command macros and register addresses \\
                     & PIC16C5x \\
...
\caption{Delivered Files (DOS) - Part 1\label{TabPackageList1}}
\end{table*}
\begin{table*}[ht]
\begin{center}\begin{tabular}{|l|l|}
\hline
...
\hline
\end{tabular}\end{center}
\caption{Delivered Files (DOS) - Part 2\label{TabPackageList2}}
\end{table*}

The package of the DPMI \marginpar{{\em DPMI}} version is significantly
smaller as it neither contains include files nor utility programs nor test
programs.  You may (you even must...) take them from the DOS version's
package.  The utility programs were not compiled as extra DPMI versions as
they would not benefit from the larger memory but still would experience
the slowdown introduced by DPMI.  The package can therefore reduce itself
to the executable of AS and the needed DOS extender (table
...
\hline
\end{tabular}\end{center}
\caption{Delivered Files (DPMI) \label{TabDPMIPackageList}}
\end{table*}

The OS/2 version of AS \marginpar{{\em OS/2}} is similar to the DPMI
version in the sense that include files and test programs weren't added to
the package (again, take them from the DOS version), but of course the
utility programs were compiled as ''native'' versions (table
\ref{TabOS2PackageList}).  AS2MSG is missing due to the missing base
(==no Borland Pascal for OS/2):
...
\hline
\end{tabular}\end{center}
\caption{Delivered Files (OS/2) \label{TabOS2PackageList}}
\end{table}

As the C version \marginpar{{\em UNIX}} is delivered in source code (in
contrast to all previous versions), its package is substantially larger
(at least to an extent that listing all files at this place would
primarily result in a waste of paper...).  For example, it additionally
includes a test suite that is quite complete in contrast to the few test
programs delivered with the DOS version.  The test suite allows to check
the correct operation of a freshly compiled version of AS.
...
usage of AS.  It is sufficient to copy all \tty{EXE} and \tty{OVR} files to a
directory listed in the \tty{PATH} environment variable.  It is irrelevant
whether you use an existing directory or create a new one for this task.
The documentation, demo programs, and include files may be placed where
you like.  Following is an example for an installation a UNIX guru would
choose:
...
c:\as\demos
\end{verbatim}
First, copy all \tty{EXE} and \tty{OVR} files from the archive to the
\tty{bin}
directory; extend your \tty{PATH} statement in \tty{AUTOEXEC.BAT} by this
directory.  Move all \tty{INC} files to the \tty{include} subdirectory.
Create a file \tty{AS.RC} in the lib subdirectory that contains the
following line:
\begin{verbatim}
-i c:\as\include
\end{verbatim}
This so-called key file tells AS where to look for include files.
You have to extend your \tty{AUTOEXEC.BAT} by the statement
\begin{verbatim}
set ASCMD=@c:\as\lib\as.rc
\end{verbatim}
to tell AS where to find the key file on invocation.  The following
section will describe what additional options can be set in the key file.
Finally, move all \tty{DOC} files to the subdirectory with the same name
and all demo assembler files to the \tty{demos} subdirectory.  That's all
folks!

As the DPMI version \marginpar{{\em DPMI}} is primarily an Addon to the
DOS version designed for special situations, it is best to previously
install the DOS version according to the scheme outlined above.
Afterwards, on can copy \tty{AS2.EXE} and the DPMI server's files to the
\tty{bin} directory (\tty{DPMIUSER.DOC} can e.g. be placed in the
\tty{DOC} directory).  If you are working on an 80286-based system, it is
possible that you get the following message the first time you try to
start ASX:
\begin{verbatim}
...
\tty{DPMIINST} once.  As \tty{DPMIINST} wants to be the sole master of the
protected mode, you probably have to remove an installed HIMEM driver
before you can do the installation (you may insert HIMEM again after the
run).  Simply follow the instructions \tty{DPMIINST} prints.  You do not
need this program after the installation any more, only the following ones
are still required:
\begin{itemize}
\item{\tty{DPMI16BI.OVL}: DPMI server}
\item{\tty{DPMILOAD.EXE}: loader for the DPMI server}
\item{\tty{RTM.EXE}: runtime control module}
\end{itemize}

The installation of the OS/2 version \marginpar{{\em OS/2}} can generally
be done just like for the DOS version, with the difference that all
\tty{EXE} and \tty{OVR} can rightaway be deleted and replaced by the OS/2
counterparts.  In contrast to DOS, the setting of the \tty{ASCMD} variable
has to be done from \tty{CONFIG.SYS} (the position of the statement is
however again arbitrary).

As the C version \marginpar{{\em UNIX}} comes in source code, the
installation is naturally a bit more complicated.  Roughly said, the
necessary steps consist of an adaption of the makefiles, starting the
compilation, doing a testrun, and installing the executables, includes,
and documentation.  The details can be found in \tty{README}.  OS/2 users
should also read \tty{README.OS2} to avoid unsatisfying failures!
...
DOS, the various code generator modules were moved for the DOS version to
an overlay file. Its existence is checked by the assembler immediately
after program startup.  If the file is not found, the program run will
find an abrupt end already at this position...  The file \tty{AS.OVR}
should be always in the same directory as the \tty{EXE}-file.  Using
overlays naturally results in slight overhead.  AS tries to reduce this by
...
\end{verbatim}
The size specification is done in megabytes and \bb{has} to be done.  The
...
The longer loading time \marginpar{{\em DPMI}} can be slightly reduced by
using the program \tty{RTMRES}.  It loads the control programs residently and
starts a new shell, resulting in a reduced loading overhead for subsequent
calls.  A simple \tty{EXIT} from the shell removes the control programs again.
...
\item{\tty{g}: This switch instructs AS to create an additional file that
      ...
      in section \ref{SectDebugFormat}.  The file's extension is \tty{MAP}.}
...
of the operand length.  Table \ref{TabAttrs} shows the allowed attribute
letters.
\begin{table*}[htb]
...
\hline
\end{tabular}\end{center}
\caption{Allowed Attribute Letters \label{TabAttrs}}
\end{table*}
\par
\bb{CAUTION:} not all instructions allow any operand length, and some
instructionss have no attribute at all. For more exact studies consult a
reasonable programmer's manual, e.g. \cite{Williams} or equivalent.
...
of integer constants can be done in different numbering systems (table
\ref{TabSystems}).
\par
\begin{table*}[htb]
...
\end{table*}
To prevent a misinterpretation of hexadecimal constants (as symbol names)
in the Intel mode, they must be started always by a digit; instead of e.g.
\tty{F0H} there must be written \tty{0F0H}.  The Motorola mode doesn't
know such problems (grin..).
...
according decimal ASCII value.  So it is possible to define by {\tt
\verb!\!3} an ETX-character.  But be careful with the definition of NUL
...
\tty{REG} works similar to \tty{SFR}, but the symbol is assigned to the
register address space.  Allowed values range from 0 to 255.
...
\tty{CHARSET} instruction.  This command expects a range specification as
first resp. first/second parameter for the characters to become
translated.  The last parameter is the range into that the characters
shall become translated.  To make things clear, two examples:
\begin{quote}{\tt
       CHARSET  '\"a',128
}\end{quote}
...
an already modified translation table can lead into other results in the
...
\hline
\tin{processor} & \tin{CODE} & \tin{DATA} & \tin{IDATA} & \tin{XDATA} & \tin{YDATA} & \tin{BITDATA} & \tin{IO} & \tin{REG} \\
\hline
\hline
68xx0      & 4G     &   ---   &   ---   &   ---   &   ---   &   ---   &   ---   &   --- \\
           & In.0   &         &         &         &         &         &         &       \\
...
Z8         & 64K    & 256     &   ---   & 64K     &   ---   &   ---   &   ---   &   --- \\
           & In.0   & In.0    &         & In.0    &         &         &         &       \\
\hline
\multicolumn{9}{|l|}{*) As the 8051 does not have any RAM beyond 80h, this value has to be} \\
\multicolumn{9}{|l|}{   adapted with ORG for the 8051 as target processor!!}\\
\hline
\multicolumn{9}{|l|}{+) As the Z180 still can address only 64K logically, the whole}\\
\multicolumn{9}{|l|}{   address space can only be reached via \tty{PHASE} instructions!}\\
\hline
\end{tabular}\end{center}
...
%%-----------
\begin{quote}
a) 68008 $\rightarrow$ 68000 $\rightarrow$ 68010 $\rightarrow$ 68012 $\rightarrow$ 68020 $\rightarrow$ 68030
\end{quote}
...
68030 that misses two instructions: \tty{CALLM} and \tty{RTM}.  The three
representors of the 683xx family have the same processor core (a slightly
reduced 68020 CPU), however completely different peripherals.
%%-----------
\begin{quote}
b) 56000
\end{quote}
%%-----------
\begin{quote}
c) PPC403 $\rightarrow$ MPC505 $\rightarrow$ MPC601 $\rightarrow$ RS6000
\end{quote}
...
which are unique in this family.  The MPC505 (a microcontroller variant
...
%%-----------
\begin{quote}
h) HD6413308 $\rightarrow$ HD6413309
\end{quote}
...
%%-----------
\begin{quote}
i) HD6475328 $\rightarrow$ HD6475348 $\rightarrow$ HD6475368 $\rightarrow$ HD6475388
\end{quote}
...
%%-----------
\begin{quote}
q) 87C750 $\rightarrow$ 8051, 8052, 80C320, 80C501, 80C502, 80C504, 80515, and\\
   80517$\rightarrow$ 80C251
\end{quote}
...
%%-----------
\begin{quote}
v) AT90S1200 $\rightarrow$ AT90S2313 $\rightarrow$ AT90S4414 $\rightarrow$
   AT90S8515
\end{quote}
...
%%-----------
\begin{quote}
w) AM29245 $\rightarrow$ AM29243 $\rightarrow$ AM29240 $\rightarrow$ AM29000
\end{quote}
...
%%-----------
\begin{quote}
x) 80C166 $\rightarrow$ 80C167,80C165,80C163
\end{quote}
...
%%-----------
\begin{quote}
y) Z80 $\rightarrow$ Z80UNDOC $\rightarrow$ Z180 $\rightarrow$ Z380
\end{quote}
...
%%-----------
\begin{quote}
z) Z8601, Z8604, Z8608, Z8630, Z8631
\end{quote}
...
%%-----------
\begin{quote}
ad) 47C00 $\rightarrow$ 470C00 $\rightarrow$ 470AC00
\end{quote}
...
%%-----------
\begin{quote}
af) 16C54 $\rightarrow$ 16C55 $\rightarrow$ 16C56 $\rightarrow$ 16C57
\end{quote}
...
%%-----------
\begin{quote}
ai) ST6210/ST6215$\rightarrow$ST6220/ST6225
\end{quote}
...
%%-----------
\begin{quote}
ak) ST9020, ST9030, ST9040, ST9050
\end{quote}
...
%%-----------
\begin{quote}
an) 320C25 $\rightarrow$ 320C26 $\rightarrow$ 320C28
\end{quote}
...
%%-----------
\begin{quote}
as) 370C010, 370C020, 370C030, 370C040 and 370C050
\end{quote}
...
{\em valid for: 680x0, XA, H8, SH7000, MSP430, TMS9900, ST7}
...
be read from a symbol with the same name.
...
\item{\tty{YDATA}: Y-addressing space of the DSP56000;}
...
{\em valid for: 680x0, H8, SH7x00, DSP56000, XA, ST7}
...
The assembler automatically adds another byte of data in case the byte sum
...
{\em valid for: ST62xx, 320C2x, 320C5x, MSP, TMS9900}
...
{\em valid for: ST62xx, 320C2x, 320C3x, 320C5x, MSP}
...
{\em valid for: PIC, 320xx, AVR, MELPS-4500}
...
segment and on the 4500, characters occupy one word.  On AVR, 17C4x in code
segment, and on 3201x/3202x, in general two characters fit into one word
(LSB first).  When in 320C3x, mode the assembler puts four characters into
one word (MSB first).  In contrast to this characters occupy two memory
locations in the data segment of the 4500. The range of integer values
corresponds to the word width of each processor in a specific segment.
This means that \tty{DATA} has the same result than \tty{WORD} on a 320C3x
(and that of \tty{SINGLE} if AS recognizes the operand as a floating-point
constant).
...
Note that when in 320C2x/5x mode the assembler assumes that a label on the
...
{\em valid for: PIC, MELPS-4500, 3201x, 320C2x, 320C5x, AVR}
...
{\em valid for: 320C2x, 320C3x, 320C5x, MSP}

\tty{BSS} works like \tty{RES}, but when in 320C2x/5x mode, the assembler
...
\ttindex{IF}\ttindex{ENDIF}\ttindex{ELSEIF}
...
not a must on the XA, so a \tty{PADDING OFF} instruction right at the
...
The documentation I had available \cite{Ti9900} unfortunately discussed
...
TI microcontrollers also do it.  Further more, TI sometimes uses \tty{Rx}
to name registers and \tty{WRx} at other places...currently both variants
...
\item{\tty{f $<$Header[,Header]$>$}: sets a list of record headers that should
      ...
      record structure described in section \ref{ChapCodeFormat}.
...
\errentry{ 110}{page might not be addressable}
               ...
               {The symbol used as an operand was not found in the memory
                page defined by an \tty{ASSUME} directive (ST62xx, 78(C)10).}
               {none}
...
\errentry{ 210}{multiple use of address register in one instruction}
               {warning}
               {An register used for the addressing is used once more in the
...
\errentry{1495}{unresolved literals (missing LTORG)}
               ...
               {none}
\errentry{1500}{order not allowed on}
               {error}
...
\errentry{1890}{invalid value pair}
               ...
               {The base-exponent pair used in the expression cannot be
                computed}
               {none}
\errentry{1900}{order must not start on this address}
               {error}
...
\errentry{1930}{constants and placeholders cannot be mixed}
               ...
               {none}
\errentry{1940}{code may only be generated in code segment}
               {error}
               {Segments not used for code can be used only for reserving
...
\item{version 1.38:
      ...
      \item{additionally supports the ST62xx series from
            SGS-Thomson (p2);}
...
\item{version 1.39:
      ...
      \item{The MELPS-7700's 'idol' 65816 is now also available
            (p4);}
      \item{the ST62xx pseudo instruction \tty{ROMWIN} has been removed
...
\begin{itemize}
\item{The structurization on top of the function level is
      ...
      portable!}
\item{Something comparable to Pascal's \tty{WITH} statement is missing.}
\item{If one limited oneself to the K\&R standard that was still
      commonplace at that time, there was practically no way for a
      compile-time check of function parameters.}
\item{There is no way to define a function that returns a string by
      \bb{value}.  This forces the programmer to use cumbersome additional
...
you own by replacing the erroneous routines in \tty{LONG.ASM}, like this:
\begin{verbatim}
; Longint shift right
; In    DX:AX = Value
;       CX    = Shift count
...
to set the predefined variable \tty{Test8086} to a value smaller than 2
...
\tty{EvalExpression} that analyzes and evaluates a formula expression.  The
...
\tty{EvalIntExpression}, \tty{EvalFloatExpression}, and
\tty{EvalStringExpression} for
...
module.  The only exported variable of importance is the flag \tty{IfAsm}
...
processors, like \tty{EQU} or \tty{ORG}.  The \tty{CPU} instruction that
switches among different target processors is also located in this module.
...
\begin{itemize}
\item{processing of options stored in an environment variable or a
      corresponding file;}
\item{return of a set that describes which parameters have not been
      processed;}
\item{distinction between normal and negated switches;}
\item{a backdoor for cases where a development environment placed on top
...
standard output:
\begin{itemize}
\item{STDERR (output channel for error messages)}
\item{STDPRN (default printer)}
\item{STDAUX (serial port)}
\end{itemize}
...
\begin{itemize}
\item{date format: order of day, month, and year and the separation
      character;}
\item{time format: 12 or 24 hour format;}
\item{translation from lower to upper case;}
\item{separation characters for fractions and thousands places in
      numbers;}
\item{currency names and formats;}
\item{collation order of characters.}
\end{itemize}
AS currently does not use all information offered by this unit (what
should an assembler do with money \tty{;-)} ), but NLS support will become
better in the future.  This is a unit that is especially suited for
use in other programs: simple inclusion delivers an \tty{UpCase} function
...
\section{A New Processor...And Now?}
...
lot more indirect but it has the advantage that there is only \bb{one}
...
\tty{CODExxxx}.  The unit head with compiler switches and \tty{Uses}
...
\tty{SwitchTo\_xxxx} (abbreviated as the ''switcher'' in the following) is a
procedure without parameters that is called by AS when the switch to
the new processor actually takes place.  \tty{AddCPU} delivers an integer
...
\tty{AddCPU} should be stored in a private variable of type \tty{CPUVar} (called
\tty{CPUxxxx} in the example above).  In case a code generator module
implements more than one processor (e.g. several processors of a
family), the module can find out which instruction subset is
currently allowed by comparing \tty{MomCPU} against the stored handles.
...
\item{\tty{ConstMode}: This variable may take the values
      \tty{ConstModeIntel},
      \tty{ConstModeMoto}, or \tty{ConstModeC} and rules which syntax has to be
...
\item{\tty{ValidSegs}: Not all processors have all address spaces defined
      ...
      the \tty{CODE} segment has to be enabled.  The complete set of allowed
      segments can be looked up the module \tty{ASMDEF} (\tty{Seg....} constants).}
...
\item{Most processors use only an address space that is much smaller
      ...
      is the \tty{ChkPC} routine found in the MCS-51/251 code generator
      \tty{Code51}.  The function should return TRUE if everything is fine
      and otherwise FALSE.  \bb{CAUTION}!  Turbo Pascal currently does not
...
as \tty{FAR} procedures (like the switcher itself); assignment to procedure
...
''registers'' \tty{R0..R255} resp. \tty{R0FF}.  AS provides the procedure variable
\tty{InternSymbol} that may be set by the code generator.  The parser calls
...
in a record of type \tty{TempResult} passed by reference when an internal
symbol has been detected.  The record's \tty{Type} field has to carry the
result type (\tty{TempInt} for integral values, \tty{TempFloat} for floating
point values, or \tty{TempString} for strings) and the result itself has to
be stored in the respective fields \tty{Int}, \tty{Float}, or \tty{Ascii}.  If the
check was without success, simply set the \tty{Type} field to
\tty{TempNone}.
...
in the unit's initialization part (right next to the \tty{AddCPU} calls):
\begin{verbatim}
   AddCopyright(
...
\subsubsection{Inserting the Code Generator Module}
...
to be done is to add the unit to the \tty{USES} list of the main module
\tty{AS.PAS}.  The definition as resident or overlay module is a decision
...
function \tty{Granularity} in \tty{TOOLS.PAS} has to be extended.  On further has
to decide which hex output format should be used by default.  Without
an extension of the \tty{CASE} statement in ProcessFile, P2HEX will
...
collected in \tty{RSC} files to make such a translation simpler.
\tty{IOERRORS.RSC} contains all I/O error messages and is used by AS and
the tools.  \tty{TOOLS.RSC} contains string constants that are used by all
tools.  Otherwise, every program has its own \tty{RSC} file.  You do not
have to care about \tty{RSC} files as long as you do not intend to
...
\bibitem{MotMic} Motorola Inc.: \\
                 {\em Microprocessor, Microcontroller and Peripheral Data.\/}
		 Vol. I+II, 1988

\bibitem{Mot81} Motorola Inc.: \\
                {\em MC68881/882 Floating Point Coprocessor User's Manual.\/}
                Second Edition, Prentice-Hall, Englewood Cliffs 1989

\bibitem{Mot51} Motorola Inc.: \\
                {\em MC68851 Paged Memory Management Unit User's Manual.\/}
...
\bibitem{SCMP} National Semiconductor: \\
               {\em SC/MP Program\-mier- und As\-sem\-bler-Hand\-buch.\/}
