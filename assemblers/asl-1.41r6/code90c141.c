   ...
   if (IsIndirect(Asc)) {
      ...
      do {
         ...
         if (!fnd) {
            ...
            if (ok) {
               if (NegFlag) DispAcc -= DispVal;
               else DispAcc += DispVal;
...
static void CodeMove(Word Index) {
   if (ArgCnt != 0) WrError(1110);
      ...
      CodeLen = 2;
      BAsmCode[0] = 0xfe;
      BAsmCode[1] = MoveOrders[Index].Code;
   }
}
   ...
   if (ArgCnt != 1) WrError(1110);
      ...
      switch (AdrType) {
         case ModReg8:
            ...
            BAsmCode[1] = ShiftOrders[Index].Code;
         ...
         case ModIdxReg:
            ...
            BAsmCode[1 + AdrCnt] = ShiftOrders[Index].Code;
            break;
         case ModDir:
            CodeLen = 3;
            BAsmCode[0] = 0xe7;
      ...
      if (OK) {
         ...
         switch (AdrType) {
            case ModReg8:
               CodeLen = 2;
               BAsmCode[0] = 0xf8 + AdrMode;
               BAsmCode[1] = BitOrders[Index].Code + HReg;
               break;
            case ModIndReg:
               CodeLen = 2;
               BAsmCode[0] = 0xe0 + AdrMode;
            ...
            case ModIdxReg:
               ...
               memcpy(BAsmCode + 1, AdrVals, AdrCnt);
               BAsmCode[0] = 0xf0 + AdrMode;
               BAsmCode[1 + AdrCnt] = BitOrders[Index].Code + HReg;
               break;
            case ModMem:
               CodeLen = 4;
               memcpy(BAsmCode + 1, AdrVals, AdrCnt);
            ...
            case ModDir:
               ...
               if (Index == 4) {
                  ...
                  BAsmCode[0] = BitOrders[Index].Code + HReg;
                  CodeLen = 2;
               }
               break;
         }
...
static void CodeAcc(Word Index) {
   if (ArgCnt != 1) WrError(1110);
      ...
      CodeLen = 1;
      BAsmCode[0] = AccOrders[Index].Code;
   }
}
   ...
   if ((WMemo("INC")) || (WMemo("DEC"))) {
      if (ArgCnt != 1) WrError(1110);
         ...
         switch (AdrType) {
            case ModReg8:
               CodeLen = 1;
               BAsmCode[0] = 0x80 + HReg + AdrMode;
               break;
            case ModReg16:
               CodeLen = 1;
               BAsmCode[0] = 0x90 + HReg + AdrMode;
               break;
            case ModIndReg:
               CodeLen = 2;
               BAsmCode[0] = 0xe0 + AdrMode;
...
static void InitFields(void) {
   ...
   MoveOrders = (FixedOrder *) malloc(sizeof(FixedOrder) * MoveOrderCnt);
   InstrZ = 0;
   AddMove("LDI", 0x58);
   AddMove("LDIR", 0x59);
   AddMove("LDD", 0x5a);
