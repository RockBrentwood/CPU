...
static void InitFields(void) {
   ...
   AccMemOrders = (FixedOrder *) malloc(AccMemOrderCnt * sizeof(FixedOrder));
   InstrZ = 0;
   AddAccMem("ADD", 0x84);
   AddAccMem("ADC", 0x80);
   AddAccMem("SUBC", 0x81);
   ...
   BitOrders = (FixedOrder *) malloc(BitOrderCnt * sizeof(FixedOrder));
   InstrZ = 0;
   AddBit("IFBIT", 0x70);
   AddBit("SBIT", 0x78);
   AddBit("RBIT", 0x68);
...
static void DecodeAdr(char *Asc, Word Mask) {
   static char *ModStrings[ModXDec + 1] = { "A", "[B]", "[B+]", "[B-]", "[X]", "[X+]", "[X-]" };
      ...
      if Memo
         ...
         if (ArgCnt != 2) WrError(1110);
            ...
            if (AdrMode != ModNone) {
               ...
               switch (AdrMode) {
                  case ModBInd:
                     BAsmCode[0] = AccMemOrders[z].Code;
                     CodeLen = 1;
                     break;
                  case ModImm:
                     BAsmCode[0] = AccMemOrders[z].Code + 0x10;
                     BAsmCode[1] = AdrVal;
                     CodeLen = 2;
                     break;
                  case ModDir:
            ...
            if (OK) {
               ...
               switch (AdrMode) {
                  case ModDir:
                     BAsmCode[0] = DirPrefix;
                     BAsmCode[1] = AdrVal;
                     BAsmCode[2] = BitOrders[z].Code + HReg;
                     CodeLen = 3;
                     break;
                  case ModBInd:
                     BAsmCode[0] = BitOrders[z].Code + HReg;
                     CodeLen = 1;
                     break;
               }
            }
