...
{\em IBM, PPC403GA und PowerPC} sind Warenzeichen der IBM Corporation.
...
Allen anderen Warenzeichen sind Eigentum der entsprechenden Eigent"umer.

\vspace{12cm}

Dieses Dokument wurde mit dem LaTeX-Satzsystem unter den Betriebssystemen
Digital Unix, Linux und OS/2, die auf AMD 5x86- und DEC Alpha-Prozessoren
...
{\parskip 0cm plus 0.1cm \tableofcontents}
...
\item{per Post: \begin{description}
                ...
                \end{description}}
\item{per E-Mail: \tty{a.arnold@kfa-juelich.de}}
\end{itemize}
...
\begin{itemize}
\item{Motorola 68000..68030,683xx inkl. Koprozessor und MMU}
\item{Motorola DSP56000}
...
Die Versionsnummer ist also nicht nur Marketing, sondern sie dr"uckt
wirklich aus, da"s dies hier keine 1.0er Version mit Kinderkrankheiten ist
(was nichts am Gewicht des in Teil \ref{SectLicense} gesagten "andert).
\par
...
STDDEFZ8.INC & Registeradresse Z8-Familie \\
...
erfahren w"urden.  Das Paket kann sich daher auf das AS-Exectutable
...
der DOS-Version ziemlich kompette Test-Suite, mit der man die Funktion der
...
\tty{demos}-Verzeichnis.  Das war\'s auch schon!
...
von AS \marginpar{{\em DPMI}} wird dadurch aber nicht beeinflu"st. Alternativ k"onnen Sie aber
auch die Ausgabe von ANSI-Sequenzen durch das Setzen der
...
\rm\end{quote}
Die Gr"o"senangabe ist in Megabytes und \bb{mu"s} gemacht werden.  Der
...
Die Parameter k"onnen grob in zwei Klassen eingeteilt werden: Schalter
und Dateispezifikationen.  Parameter dieser beiden Klassen k"onnen
...
\begin{itemize}
\item{\tty{g}: Bei Angabe dieses Schalters erzeugt AS zus"atzlich eine Datei, die
      Debug-Informationen f"ur dieses Programm enth"alt.  Dazu geh"ort
      zum einen die Symboltabelle, zum anderen eine Zuordnung von
      Quellzeilen zu Maschinenadressen.  Eine genauere Beschreibung des
      benutzten Dateiformates findet sich in Abschnitt
      \ref{SectDebugFormat} .  Die Endung der Datei ist MAP .}
...
\item{\tty{D $<$Symbolliste$>$}: Symbole definieren. Die hinter dieser Option
      ...
      oder interne Funktionen beinhalten, jedoch \bf KEINE \rm anderen
...
werden.  F"ur solche F"alle kann auf eine sog. \it Key-\rm Datei
...
als auch Argument in \bf einer \rm Zeile stehen m"ussen.  Der Name der
...
dem Befehl  mit einem Punkt angeh"angtes Attribut zur Beschreibung
der Operandengr"o"se.  Tabelle \ref{TabAttrs} zeigt die erlaubten
Attributbuchstaben.
\begin{table*}[htb]
...
\hline
\end{tabular}\end{center}
\caption{Erlaubte Attributbuchstaben\label{TabAttrs}}
\end{table*}
\par
{\bf ACHTUNG :} Nicht alle Befehle erlauben alle Operandengr"o"sen, und
manche Befehle haben gar kein Attribut.  Zum genaueren Studium greife man
auf ein vern"unftiges Programmierhandbuch zur"uck, z.B. in \cite{Williams}
f"ur die 68000er-Familie.
\par
...
voneinander erfolgt ausschlie"slich durch Kommas (Ausnahme: DSP56000,
...
\par
Im Feld \tt Zeile \rm wird die Zeilennummer bezogen auf die jeweilige Datei
...
Schr"agstrich im Feld \tt Adresse\rm.
\par
Der erzeugte Code selber steht dahinter im Feld \tt Code \rm in
...
\par
Im Feld \tt Quelle \rm schlu"sendlich wird die Zeile aus der Quelldatei in
...
einzelne Teile wieder einschalten, falls man es in der \tt ASCMD\rm -Variablen
...
Motorola- bzw. C-Modus kennt derartige Probleme nicht (\it hihihi!\rm).
\par
...
\end{verbatim}
lautet. \bf ACHTUNG! \rm Der Assembler versucht eine Konstante zuerst als
...
dies durch Dummy-Nachkommastellen erreichen, z.B. \tt 2.0 \rm anstelle
\tt 2\rm.
\par
...
Operator wird also \it zuletzt \rm ausgewertet.  Die Reihenfolge der
...
Speziell bei groﬂen Programmen kann es zu einer interessanten Situation
...
\begin{enumerate}
\item{Weisen Sie AS explizit darauf hin, daﬂ er f"ur die \tty{LDD}-Befehle
...
\par
\bf ACHTUNG! \rm Ist keiner dieser Schalter angegeben, so wird auch keine
...
um das Symbol einem bestimmten Adre"sraum zuordnen.
...
\tt SFR\rm , nur da"s \tt XSFR \rm ein Symbol im X-Adre"sraum definiert
und \tt YSFR \rm entsprechend eines im Y-Adre"sraum.  Der erlaubte
...
Die Funktion des \tt LABEL\rm -Befehls ist identisch zu \tt EQU\rm, nur
wird das Symbol nicht typenlos, sondern erh"alt das Attribut ,,Code''.
\tt LABEL \rm wird genau f"ur einen Zweck ben"otigt: Labels in Makros
sind normalerweise lokal, also nicht au"serhalb des Makros zugreifbar.
Mit einem \tt EQU\rm -Befehl kann man sich zwar aus der Aff"are ziehen,
die Formulierung
\begin{verbatim}
<Name>   label    $
\end{verbatim}
...
Bits k"onnen dann von den Befehlen \tt SBIT0, SBIT1, CMPBIT, JBIT0 \rm
...
\tty{REG} arbeitet analog zu \tty{SFR}, nur wird das Symbol dem
Register-Adre"sbereich zugeordnet.  Erlaubte Werte sind 0..255.
...
umgemappt werden.
\par
\bf ACHTUNG! \rm \tty{CHARSET} beeinflu"st nicht nur im Speicher abgelegte
...
%%-----------
\begin{quote}
a) 68008 $\rightarrow$ 68000 $\rightarrow$ 68010 $\rightarrow$ 68012
$\rightarrow$ 68332 $\rightarrow$ 68340 $\rightarrow$ 68360
$\rightarrow$ 68020 $\rightarrow$ 68030
\end{quote}
...
68030 dar, dem 2 Befehle fehlen: \tty{CALLM} und \tty{RTM}.  Die drei
...
%%-----------
\begin{quote}
d) 6800 $\rightarrow$ 6301 $\rightarrow$ 6811
\end{quote}
...
%%-----------
\begin{quote}
h) HD6413308 $\longrightarrow$ HD6413309
\end{quote}
...
%%-----------
\begin{quote}
i) HD6475328 $\longrightarrow$ HD6475348 $\longrightarrow$
...
%%-----------
\begin{quote}
j) SH7000 $\longrightarrow$ SH7600
\end{quote}
...
sowie Multiplizier/Addier-Befehle).
%%-----------
\begin{quote}
k) 6502 $\rightarrow$ 65(S)C02 / MELPS740
\end{quote}
...
%%-----------
\begin{quote}
q) 87C750 $\rightarrow$ 8051, 8052, 80C320, 80C501, 80C502, 80C504,
   80515 und 80517 $\rightarrow$ 80C251
\end{quote}
...
%%-----------
\begin{quote}
r) 8096 $\rightarrow$ 80196 $\rightarrow$ 80196N $\rightarrow$ 80296
\end{quote}
...
%%-----------
\begin{quote}
t) 8086 $\rightarrow$ 80186 $\rightarrow$ V30 $\rightarrow$ V35
\end{quote}
...
%%-----------
\begin{quote}
w) AT90S1200 $\rightarrow$ AT90S2313 $\rightarrow$ AT90S4414 $\rightarrow$
   AT90S8515
\end{quote}
...
%%-----------
\begin{quote}
x) AM29245 $\rightarrow$ AM29243 $\rightarrow$ AM29240 $\rightarrow$ AM29000
\end{quote}
...
%%-----------
\begin{quote}
y) 80C166 $\longrightarrow$ 80C167,80C165,80C163
\end{quote}
...
%%-----------
\begin{quote}
z) Z80 $\rightarrow$ Z80UNDOC $\rightarrow$ Z180 $\rightarrow$ Z380
\end{quote}
...
%%-----------
\begin{quote}
ae) 47C00 $\rightarrow$ 470C00 $\rightarrow$ 470AC00
\end{quote}
...
%%-----------
\begin{quote}
ag) 16C54 $\rightarrow$ 16C55 $\rightarrow$ 16C56 $\rightarrow$ 16C57
\end{quote}
...
%%-----------
\begin{quote}
aj) ST6210/ST6215 $\rightarrow$ ST6220/ST6225
\end{quote}
...
%%-----------
\begin{quote}
ao) 320C25 $\rightarrow$ 320C26 $\rightarrow$ 320C28
\end{quote}
...
az) 78070
\end{quote}
Dies ist das einizige, mir momentan vertraute Mitglied der
...
{\em\begin{tabbing}
G"ultigkeit: \= 680x0, FPU auch 80x86, SUPMODE auch TLCS-900, SH7000, 29K,\\
             \> XA und PowerPC
\end{tabbing}}
...
{\em G"ultigkeit: 680x0, XA, H8, SH7000, TMS9900, MSP430, ST7}
...
ist analog zu den vorherigen Befehlen \tty{ON} oder \tty{OFF} erlaubt,
und die augenblickliche Einstellung kann aus dem gleichnamigen Symbol
ausgelesen werden.
...
und den Parametern \tt ON \rm oder \tt OFF \rm teilt man AS mit, da"s der
folgende Code im Maximum- oder Minimum-Modus abl"auft.  Die momentane
Einstellung kann aus der Variablen \tt INMAXMODE \rm ausgelesen werden.
...
\item{\tty{XDATA}: im extern angeschlossenen RAM liegende Daten oder
             X-Adre"sraum beim DSP56000;}
\item{\tty{YDATA}: Y-Adre"sraum beim DSP56000;}
...
\hspace{1.5cm} \= \hspace{3cm} \= \hspace{2cm} \= \kill
      \> CPU     \> 8051 \> ; MCS-51-Code \\
\\
      \> SEGMENT \> code \> ; Testcodest"uck \\
\\
...
Mit dem Befehl \tt SAVE \rm legt der Assembler den Inhalt folgender
Variablen auf einen internen Stapel:
\begin{itemize}
\item{momentan gew"ahlter Prozessortyp (mit \tt CPU \rm gesetzt);}
\item{momentan aktiver Speicherbereich (mit \tt SEGMENT \rm gesetzt);}
\item{Flag, ob Listing ein- oder ausgeschaltet ist (mit \tt LISTING \rm
      gesetzt);}
\item{Flag, ob Expansionen folgender Makos im Listing ausgegeben
      werden sollen (mit \tt MACEXP \rm gesetzt).}
\end{itemize}
Mit dem Gegenst"uck \tt RESTORE \rm wird entsprechend der zuletzt
...
Werte auf einmal gesetzt werden und es wird \bf kein \rm Code erzeugt, der
...
zugewiesen werden.  Dieser Befehl erzeugt jedoch \bf keinen \rm Code, um
...
wurde und man sich im folgenden explizite \tt CS:\rm -Anweisungen sparen
kann.
\par
G"ultige Argumente hinter dem Doppelpunkt sind \tt CODE, DATA \rm und \tt
NOTHING\rm .  Letzterer Wert dient dazu, AS mitzuteilen, da"s das
Segmentregister keinen f"ur AS verwendbaren Wert enth"alt.
Vorinitialisiert sind folgende \tt ASSUME\rm s :
\begin{verbatim}
...
Die XA-Familie besitzt einen Datenadre"sraum von 16 Mmyte, ein Proze"s
...
Hat die Variable \tt VARI \rm z.B. den Wert 456h, so setzt
\begin{verbatim}
...
Anstelle eines Symbols kann auch schlicht \tt NOTHING \rm angegeben
...
Voraussetzung, da"s man ihm "uber einen \tt ASSUME-\rm Befehl den
...
{\em G"ultigkeit: 680x0, H8, SH7000, DSP56000, XA, ST7}
...
ein weiteres Byte angef"ugt, um die Wortausrichtung von Daten zu
...
{\em G"ultigkeit: 680x0, H8, SH7x00, DSP56000, XA, ST7}
...
\par
Beim 56000 ist die Operandengr"o"se auf Worte (\`a 24 Bit) festgelegt,
...
Wortkonstanten gef"ullt werden, die durch den zweiten Parameter ange-
geben wird.  Die Maximalgr"o"se des Blocks betr"agt 1024 Elemente f"ur
...
ablegt, versieht \tty{ASCIZ} automatisch \it jeden \rm angegebenen String
...
abgelegt.  Beachten Sie jedoch, da"s im Gegensatz zum Originalassembler
...
\begin{itemize}
\item{\tt EXPAND/NOEXPAND \rm : legen fest, ob bei der sp"ateren
      Verwendung diese Makros der expandierte Code mit angezeigt
      werden soll.  Default ist der durch den Pseudobefehl \tt MACEXP
      \rm festgelegte Wert.}
...
\item{\tty{GLOBAL[:Sektionsname]} : legt fest, da"s neben diesem
      ...
      nicht beeinflu"st.}
\item{\tt EXPORT/NOEXPORT \rm : legen fest, ob die Definition dieses
      Makros in einer getrennten Datei abgelegt werden soll, falls
      die Kommandozeilenoption \tt -M \rm gegeben wurde.  Auf diese
...
\begin{quote}
\it It's not a bug, it's a feature! \rm
\end {quote}
...
srl     macro   op,n      ; Schieben um n Stellen
        ...
        endm
\end{verbatim}
Fortan hat der \tt SRL\rm -Befehl einen weiteren Parameter...
...
\par
\bf ACHTUNG! \rm Analog wie bei Makros kann man mit der Definition von
...
"ubersprungen werden.  Diese Befehle sind also \it nicht \rm mit den
...
,,Default-Zweig''.  Wichtig ist, da"s von den Bl"ocken immer nur \it einer \rm
...
nur \it ein \rm Zweig ausgef"uhrt, und zwar bei Mehrdeutigkeiten der erste.
\par
...
die Default-Vorgabe.
\par
Zwischen der Bedeutung von \tt MACEXP \rm f"ur Makros und der f"ur alle
anderen makroartigen Konstrukte (z.B. \tt REPT\rm) besteht ein subtiler
...
\tt MACEXP \rm direkt auf alle anderen Konstrukte, die ,,vor Ort''
...
will, andere aber sehr wohl noch.  \tt MACEXP \rm dient hier als
Default f"ur das bei der Definition des Makros zu setzende Flag, der mit
den Steuerparametern \tt NOEXPAND/EXPAND \rm "ubersteuert werden kann.
...
Papierverbrauch nicht ins Unerme"sliche zu steigern.  \bf ACHTUNG! \rm
...
Eine \it Sektion \rm stellt einen durch spezielle Befehle eingerahmten
...
zu \tt ENDSECTION \rm ist optional, es darf auch weggelassen werden; Falls
es weggelassen wird, zeigt AS den Namen der Sektion an, der er das
\tt ENDSECTION \rm zugeordnet hat.  Code in einer Sektion wird von AS genauso
...
\par
Da ein \tt SECTION\rm -Befehl von selber kein Label definiert, besteht hier
...
Die \tt PUBLIC\rm -Anweisung erlaubt es, die Zugeh"origkeit eines Symbols
zu einer bestimmten Sektion zu ver"andern.  Es ist m"oglich, mit einem
\tt PUBLIC\rm -Befehl mehrere Symbole zu bearbeiten, ohne Beschr"ankung
...
einsortiert wird, mu"s diese Anweisung \bf vor \rm der Definition des
Symbols erfolgen.  Alle \tt PUBLICs \rm werden von AS in einer Liste
...
Als Spezialwert f"ur \tt $<$Sektion$>$ \rm sind die im vorigen Abschnitt
...
\tty{GLOBAL}-Anweisung m"oglich.  Die Syntax von \tt GLOBAL \rm ist
der von \tt PUBLIC \rm identisch, das Symbol bleibt aber lokal, anstatt
...
\end{verbatim}
stehen.
Damit \tt FORWARD \rm Sinn macht, mu"s es nicht nur vor der Definition des
...
nicht gefunden, so ist dies ein \it fataler \rm Fehler, d.h. der Assembler
bricht sofort ab.
\par
...
\end{verbatim}
sind also "aquivalent.  \bf ACHTUNG! \rm Wegen dieser Wahlfreiheit ist
...
festzulegen, k"onnte man sie einmal mit \tt EQU \rm in einem Symbol
ablegen, es geht aber auch interaktiv mit \tt READ\rm :
\begin{verbatim}
        IF      MomPass=1
...
\tt ORI \rm auch die allgemeineren Mnemonics \tty{MOVE, AND} und \tty{OR}
...
Feature: Setzt man mit dem Befehl \tt SET \rm das Bit 5 des
...
(\tt SET\rm ...Befehle mit Akkuadressierung...\tt CLT\rm) genutzt werden.
\par
...
\item{Die Befehle \tt SEB\rm, \tt CLB\rm, \tt BBC\rm, \tt BBS\rm,
      \tt CLM\rm, \tt SEM\rm, \tt PSH\rm, \tt PUL \rm und \tt LDM \rm
      fehlen beim 65816.  An deren Stelle in der Code-Tabelle finden
      sich \tt TSB\rm, \tt TRB\rm, \tt BIT\rm, \tt CLD\rm, \tt SED\rm,
      \tt XBA\rm, \tt XCE \rm und \tt STZ\rm.}
\end{itemize}
...
\end{tabular}\end{center}
\par
Besonders t"uckisch sind die Befehle \tt PHB\rm, \tt PLB \rm und \tt TSB \rm:
...
man ihn "uber den \tt ASSUME\rm -Befehl "uber den Inhalt einiger
...
Ziel von Operationen auch weggelassen werden darf, anstelle von \tt
LDA A,\#0 \rm darf also z.B. auch einfach \tt LDA \#0 \rm geschrieben
werden.
\par
Ein echtes Bonbon in dem Befehlssatz sind dagegen die Blocktransferbefehle
\tt MVN \rm und \tt MVP\rm .  Etwas eigenartig ist nur die Adre"sangabe:
...
\tt SEL MB0 \rm bzw. \tt SEL MB1 \rm vorgegeben wird.
...
\tt SEL MBx\rm -Befehle sollte daher nicht notwendig sein (obwohl sie m"oglich
...
\tt SEL MB0\rm-Befehl vor dem Sprung ein, mit der Folge, da"s der
...
etwas abzuhelfen, ist in den Include-Dateien das Makro \tt USING \rm definiert,
dem als Parameter die Symbole \tt Bank0..Bank3 \rm gegeben werden k"onnen.
Das Makro belegt daraufhin die Symbole \tt AR0..AR7 \rm mit den passenden
...
\item{Verwenden Sie nur das \tt CODE\rm -Segment, und legen Sie auch alle
      Variablen darin ab.}
\item{Alle Segmentregister werden von DOS auf das Codesegment
      vorinitialisiert.  Ein \tt ASSUME DS:CODE, SS:CODE \rm am
      Programmanfang ist daher notwendig.}
\item{DOS l"adt den Code ab Adresse 100h.  Ein \tt ORG \rm auf diese
...
\it ein \rm Codesegment mit maximal 64 KByte und ein ebenfalls h"ochstens 64
...
\hline
\hline
MASM       & \tt mov ax,offset vari \rm & \tt mov ax,vari \rm    \\
           & \tt lea ax,vari \rm        & \tt mov ax,[vari] \rm  \\
           & \tt lea ax,[vari] \rm      &                        \\
           &                            &                        \\
AS         & \tt mov ax,vari \rm        & \tt mov ax,[vari] \rm  \\
           & \tt lea ax,[vari]          &                        \\
           &                            &                        \\
\hline
\end{tabular}\end{center}
...
die 68000-Familie.  Dies bedeutet dummerweise auch, da"s defaultm"a"sig
...
\tty{PADDING OFF} am Anfang schon fast obligatorisch...
\par
...
Diese Prozessoren k"onnen in zwei Betriebsarten laufen, einmal im \it
Minimum\rm -Modus, der weitgehende Z80- und TLCS-90-Quellcodekompatibilit"at
bietet, und zum anderen im \it Maximum-\rm Modus, in dem der Prozessor
...
mit dem Befehl \tt MAXMODE \rm (siehe dort) mitteilen, in welcher Betriebsart
...
XIX..XIZ und XSP sind \bf immer \rm 32 Bit breit und m"ussen zur Adressierung
...
\item{Unter TAS900 k"onnen Integerkonstanten sowohl in C-Notation (mit
      ...
      Default-Einstellung \bf nur \rm die Intel-Notation.  Mit dem
...
\begin{itemize}
\item{Bei den Befehlen \tt LDA, JP \rm und \tt CALL \rm verlangt TAS, da"s
      Adre"sausdr"ucke wie \tt XIX+5 \rm nicht geklammert sein d"urfen, wie
...
\item{Bei den bedingten Befehlen \tt JP, CALL, JR \rm und \tt SCC \rm stellt
      ...
      \tt jp (xix+5) \rm anstelle von \tt jp t,(xix+5) \rm).}
\item{AS erlaubt beim \tt EX\rm -Befehl auch Operandenkombinationen, die
      ...
      k"onnen.  Kombinationen wie \tt EX f`,f \rm oder \tt EX wa,(xhl) \rm
      werden damit m"oglich.  TAS900 hingegen l"a"st nur die ,,reine Lehre''
      zu.}
\item{AS erlaubt, bei den Befehlen \tt INC \rm und \tt DEC \rm die Angabe
...
\begin{itemize}
\item{Die Befehle \tt MAX \rm und \tt NORMAL \rm sind f"ur die L-Version
      nicht erlaubt, der \tt MIN\rm -Befehl ist f"ur die Vollversion
      gesperrt.}
\item{Die L-Version kennt den Normal-Stapelzeiger \tt XNSP/NSP \rm nicht,
      daf"ur das Steuerregister \tt INTNEST\rm.}
\end{itemize}
Die Befehle \tt SUPMODE \rm und \tt MAXMODE \rm werden nicht beeinflu"st,
...
habe ich vorerst so gel"ost, da"s der Ausdruck von \bf hinten \rm an nach
...
erreicht sein, was AS unterst"utzen kann.  Neben dem \tt ASSUME\rm -Befehl
...
\begin{itemize}
\item{der \tt B\rm -Befehl, der die jeweils optimale Version des
      Sprungbefehls (\tt BSS, BS oder BSL\rm) automatisch w"ahlt;}
\item{\tt LD \rm in der Variante HL mit immediate;}
\item{\tt ROLC \rm und \tt RORC \rm mit einer Schiebeamplitude $>$1.}
\end{itemize}
...
f"ur die vollst"andige Adresse war.  AS verwendet f"ur die Befehle \tt CALL
\rm und \tt GOTO \rm die gleiche Automatik, d.h. setzt die PA-Bits im
...
einblenden.  Weil ich nicht die ganze Mimik des \tt ASSUME\rm -Befehles
...
\tt AND\rm -Befehl, aber kein \tt OR\rm...von \tt XOR \rm gar nicht zu
...
\tt SST\rm, der nur in die obere Bank schreiben kann). Der Programmierer
...
\tt SUBC\rm -Anweisung ben"otigt zur Ausf"uhrung intern mehr als einen
Takt, das Steuerwerk arbeitet jedoch schon an dem n"achsten Befehl weiter.
Im auf ein \tt SUBC \rm folgenden Befehl darf deshalb nicht auf den
Akkumulator zugegriffen werden.  AS nimmt hier \bf keine \rm Pr"ufung vor!
...
\begin{quote}\tt
  ...
  DOUBLE, EFLOAT, BFLOAT \rm und \tt TFLOAT
\rm\end{quote}
...
Leider war die Dokumentation, die ich zur Verf"ugung hatte
\cite{Ti9900}, eine Beschreibung, die die Assemblersyntax nur am Rande
streifte.  Ich habe daher an der einen oder anderen Stelle etwas raten
m"ussen...zum Beispiel bei der Operandenreihenfolge: Da die meisten
anderen TI-Mikrokontroller die Quelle als ersten Operanden schreiben,
habe ich dieses auch f"ur dem 9900 "ubernommen.  Weiterhin wechselt
TI mit der Registerbezeichnung zwischen Rx und WRx...vorerst ist beides
zugelassen.
...
als Argument f"ur die Pseudobefehle \tt SBIT0, SBIT1, CMPBIT, JBIT0 \rm
und \tt JBIT1 \rm verwenden kann.  Diese werden in die Befehle
\tt OR, AND, XOR, BTJZ \rm und \tt BTJO \rm mit einer passenden Bitmaske
"ubersetzt.
\par
...
\hline
\end{tabular}\end{center}
\caption{Returncodes der Dientsprogramme\label{TabToolReturns}}
\end{table*}
...
\par
\bf ACHTUNG! \rm An dieser Stelle sind keine Jokerzeichen erlaubt! Falls mit
...
\begin{itemize}
\item{\tt f $<$Header[,Header...]$>$\rm: gibt eine Liste von Header-IDs
...
\par
P2HEX kennt 7 verschiedene Zielformate, die "uber den
...
\item{16-Bit Intel-Hex (MCS-86, \tty{-F Intel16\rm})}
...
16-Bit-Texas-Signalprozessoren und Intel-Hex f"ur den Rest.
Je nach Breite der Startadresse kommen bei S-Record Records
der Typen 1,2 oder 3 zum Einsatz, jedoch nie in einer Gruppe gemischt.
Die Intel-, Tektronix- und MOS-Formate sind auf 16 Bit-Adressen beschr"ankt,
das 16-Bit Intel-Format reicht 4 Bit weiter.  L"angere Adressen werden von
P2HEX mit einer Warnung gemeldet und abgeschnitten(!).
F"ur die PICs k"onnen die drei von Microchip
spezifizierten Varianten des Intel-Hex-Formates erzeugt werden, und
zwar mit dem Schalter
\begin{verbatim}
  -m <0..3>
\end{verbatim}
...
soll, die Endadresse die der letzten Speicherzelle im Fenster, \it nicht \rm
...
\par
\bf ACHTUNG! \rm Die Splittung "andert nichts an den absoluten Adressen, die
...
den Parameter \bf i \rm mit einer nachfolgenden Ziffer:
\begin{description}
\item[0]{ :00000001FF}
\item[1]{ :00000001}
\item[2]{ :0000000000}
...
\begin{itemize}
\item{\tt l $<8-Bit-Zahl>$\rm: gibt den Wert an, mit dem unbenutzte
...
\item{\tt s\rm: weist das Programm an, eine Pr"ufsumme "uber die Bin"ardatei zu
...
\item{\tt m\rm:  f"ur den Fall, da"s ein Prozessor mit 16- oder 32-Bit-Datenbus
...
Eintrag angelegt werden (\tt Options/Tools/New\rm).  Tragen Sie in die
einzelnen Felder folgende Werte ein :
\begin{itemize}
\item{Title: \tt\verb!~!M\verb!~!akroassembler\rm}
\item{Program path: \tt AS\rm}
\item{Command line: \tt -E !1 \$EDNAME \$CAP MSG(AS2MSG) \$NOSWAP \$SAVE ALL\rm}
\item{bei Bedarf einen Hotkey zuordnen (z.B. Shift-F7)}
\end{itemize}
Die Option \tt -E \rm sorgt daf"ur, da"s Turbo-Pascal nicht mit STDOUT und
...
nun ,,browsen'' kann.  Mit \bf Ctrl-Enter \rm springt man eine fehlerhafte
...
\begin{description}
\item[0]   {Displacement=0, "uberfl"ussig
            \begin{description}
            \item[Fehlertyp]{:\\Warnung}
            \item[Ursache]{:\\bei 680x0-,6809- und COP8-Prozessoren: Das Displacement
            ...
            \item[Argument]{:\\keines}
            \end{description}}
\item[10]  {Kurzadressierung m"oglich
            \begin{description}
            \item[Fehlertyp]{:\\Warnung}
            \item[Ursache]{:\\bei 680x0-, 6502- und 68xx-Prozessoren k"onnen
            ...
            \item[Argument]{:\\keines}
            \end{description}}
\item[20]  {kurzer Sprung m"oglich
            \begin{description}
            \item[Fehlertyp]{:\\Warnung}
            \item[Ursache]{:\\Bei 680x0 und 8086-Prozessoren kann der Sprung
            ...
            \item[Argument]{:\\keines}
            \end{description}}
\item[30]  {kein Sharefile angelegt, SHARED ignoriert
            \begin{description}
            \item[Fehlertyp]{:\\Warnung}
            \item[Ursache]{:\\Es wurde eine \tty{SHARED}-Anweisung gefunden, es
                 wurde aber keine Kommandozeilenoption angegeben, um eine
                 Shared-Datei zu erzeugen.}
            \item[Argument]{:\\keines}
            \end{description}}
\item[40]  {FPU liest Wert evtl. nicht korrekt ein ($>$=1E1000)
            \begin{description}
            \item[Fehlertyp]{:\\Warnung}
            \item[Ursache]{:\\Das BCD-Gleitkommaformat der 680x0-Koprozessoren
            ...
            \item[Argument]{:\\keines}
            \end{description}}
\item[50]  {Privilegierte Anweisung
            \begin{description}
            \item[Fehlertyp]{:\\Warnung}
            \item[Ursache]{:\\Es wurde eine Anweisung benutzt, die nur im
                 Supervisor-Mode zul"assig ist, obwohl dieser nicht mittels
                 \tty{SUPMODE ON} vorher explizit angezeigt wurde.}
            \item[Argument]{:\\keines}
            \end{description}}
\item[60]  {Distanz 0 nicht bei Kurzsprung erlaubt (NOP erzeugt)
            \begin{description}
            \item[Fehlertyp]{:\\Warnung}
            \item[Ursache]{:\\Ein kurzer Sprung mit der Distanz 0 ist bei
            ...
            \item[Argument]{:\\keines}
            \end{description}}
\item[70]  {Symbol aus falschem Segment
            \begin{description}
            \item[Fehlertyp]{:\\Warnung}
            \item[Ursache]{:\\Das in dem Operanden benutzte Symbol
                 ist aus einem Adre"sraum, der nicht mit dem benutzten
                 Befehl bearbeitet werden kann.}
            \item[Argument]{:\\keines}
            \end{description}}
\item[75]  {Segment nicht adressierbar
            \begin{description}
            \item[Fehlertyp]{:\\Warnung}
            \item[Ursache]{:\\Das in dem Operanden benutzte Symbol
                 ist aus einem Adre"sraum, der mit keinem der Segmentregister
                 des 8086 adressiert werden kann.}
            \item[Argument]{:\\Name des nicht adressierbaren Segments}
            \end{description}}
\item[80]  {"Anderung des Symbolwertes erzwingt zus"atzlichen Pass
            \begin{description}
            \item[Fehlertyp]{:\\Warnung}
            \item[Ursache]{:\\Ein Symbol hat einen anderen Wert zugewiesen
                 bekommen als im vorhergehenden Pass.  Diese Warnung wird
                 nur ausgegeben, falls die \tty{r}-Option angegeben wurde.}
            \item[Argument]{:\\Der Name des fraglichen Symbols}
            \end{description}}
\item[90]  {"Uberlappende Speicherbelegung
            \begin{description}
            \item[Fehlertyp]{:\\Warnung}
            \item[Ursache]{:\\Bei der Bildung der Belegungsliste wurde
            ...
            \item[Argument]{:\\keines}
            \end{description}}
\item[100] {keine CASE-Bedingung zugetroffen
            \begin{description}
            \item[Fehlertyp]{:\\Warnung}
            \item[Ursache]{:\\bei einem \tty{SWITCH}..\tty{CASE}-Konstrukt ohne
                 \tty{ELSECASE}-Zweig traf keiner der \tty{CASE}-Zweige zu.}
            \item[Argument]{:\\keines}
            \end{description}}
\item[110] {Seite m"oglicherweise nicht adressierbar
            \begin{description}
            \item[Fehlertyp]{:\\Warnung}
            \item[Ursache]{:\\Das in dem Operanden benutzte Symbol
            ...
            \item[Argument]{:\\keines}
            \end{description}}
\item[120] {Registernummer mu"s gerade sein
            \begin{description}
            \item[Fehlertyp]{:\\Warnung}
            \item[Ursache]{:\\Die Hardware erlaubt nur ein Registerpaar
                 zu verketten, dessen Startadresse gerade ist (RR0, RR2...,
                 nur Z8).}
            \item[Argument]{:\\keines}
            \end{description}}
\item[130] {veralteter Befehl
            \begin{description}
            \item[Fehlertyp]{:\\Warnung}
            \item[Ursache]{:\\Der verwendete Befehl ist zwar noch
            ...
            \item[Argument]{:\\keines}
            \end{description}}
\item[140] {Nicht vorhersagbare Ausf"uhrung dieser Anweisung
            \begin{description}
            \item[Fehlertyp]{:\\Warnung}
            \item[Ursache]{:\\Die verwendete Adressierungsart ist bei
            ...
            \item[Argument]{:\\keines}
            \end{description}}
\item[150] {Lokaloperator au"serhalb einer Sektion "uberfl"ussig
            \begin{description}
            \item[Fehlertyp]{:\\Warnung}
            \item[Ursache]{:\\Ein vorangestellter Klammeraffe dient
            ...
            \item[Argument]{:keines}
            \end{description}}
\item[160] {sinnlose Operation
            \begin{description}
            \item[Fehlertyp]{:\\Warnung}
            \item[Ursache]{:\\Die Anweisung ergibt entweder "uberhaupt
            keinen Sinn oder kann auf andere Weise schneller und k"urzer
            ausgef"uhrt werden.}
            \item[Argument]{:keines}
            \end{description}}
\item[170] {unbekannter Symbolwert erzwingt zus"atzlichen Pass
            \begin{description}
            \item[Fehlertyp]{:\\Warnung}
            \item[Ursache]{:\\AS vermutet eine Vorw"artsreferenz eines
            ...
            \item[Argument]{:Der Name des fraglichen Symbols}
            \end{description}}
\item[180] {Adresse nicht ausgerichtet
            \begin{description}
            \item[Fehlertyp]{:\\Warnung}
            \item[Ursache]{:\\Eine Adresse ist nicht ein mehrfaches der
            ...
            \item[Argument]{:\\keines}
            \end{description}}
\item[190] {I/O-Adresse darf nicht verwendet werden
            \begin{description}
            \item[Fehlertyp]{:\\Warnung}
            \item[Ursache]{:\\Der verwendete Adressierungsmodus oder die
            ...
            \item[Argument]{:\\keines}
            \end{description}}
\item[200] {m"ogliche Pipeline-Effekte
            \begin{description}
            \item[Fehlertyp]{:\\Warnung}
            \item[Ursache]{:\\Ein Register wird in einer Befehlsfolge so
            ...
            steht.}
            \item[Argument]{:\\das die Verklemmung verursachende Register}
            \end{description}}
\item[210] {mehrfache Adre"sregisterbenutzung in einer Anweisung
            \begin{description}
            \item[Fehlertyp]{:\\Warnung}
            \item[Ursache]{:\\Ein Adre"sregister wird in mehreren
            ...
            \item[Argument]{:\\das mehrfach verwendete Register}
            \end{description}}
\item[220] {Speicherstelle ist nicht bitadressierbar
            \begin{description}
            \item[Fehlertyp]{:\\Warnung}
            \item[Ursache]{:\\Mit einer \tty{SFRB}-Anweisung wurde
            versucht, eine Speicherstelle als bitadressierbar zu
            deklarieren, die aufgrund der Architektur des 8051 nicht
            bitadressierbar ist.}
            \item[Argument]{:\\keines}
            \end{description}}
\item[230] {Stack ist nicht leer
            \begin{description}
            \item[Fehlertyp]{:\\Warnung}
            \item[Ursache]{:\\Am Ende eines Durchlaufes ist ein vom
            Programm definierter Stack nicht leer.}
            \item[Argument]{:\\der Name des Stacks sowie seine Resttiefe}
            \end{description}}
\item[240] {NUL-Zeichen in Strings, Ergebnis undefiniert
            \begin{description}
	    \item[Fehlertyp]{:\\Warnung}
	    \item[Ursache]{:\\Eine String-Konstante enth"alt ein
	    ...
	    \item[Argument]{:\\keines}
            \end{description}}
\item[250] {Befehl "uberschreitet Seitengrenze
            \begin{description}
	    \item[Fehlertyp]{:\\Warnung}
	    \item[Ursache]{:\\Ein Befehl steht zu Teilen auf
	    ...
	    \item[Argument]{:\\keines}
            \end{description}}
\item[1000]{Symbol doppelt definiert
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Einem Symbol wurde durch ein Label oder
	         \tty{EQU}, \tty{PORT}, \tty{SFR}, \tty{LABEL},
		 \tty{SFRB} oder \tty{BIT} ein neuer  Wert zugewiesen, dies
                 ist aber nur bei \tty{SET/EVAL} erlaubt.}
            \item[Argument]{:\\Name des fraglichen Symbols, bei eingeschalteter
	    Querverweisliste zus"atzlich die Zeile der ersten Definition}
            \end{description}}
\item[1010]{Symbol nicht definiert
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Ein benutztes Symbol ist auch im 2.Pass noch
                 nicht in der Symboltabelle enthalten.}
            \item[Argument]{:\\Name des nicht gefundenen Symbols}
            \end{description}}
\item[1020]{Ung"ultiger Symbolname
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Ein Symbolname entspricht nicht den Bedingungen
                 f"ur einen g"ultigen Symbolnamen.  Beachten Sie, da"s f"ur
                 Makro-und Funktionsparameter strengere Regeln gelten!}
            \item[Argument]{:\\der fehlerhafte Symbolname}
            \end{description}}
\item[1090]{Ung"ultiges Format
            \begin{description}
            \item[Fehlertyp:]{:\\Fehler}
            \item[Ursache]{:\\Das benutzte Befehlsformat existiert bei diesem
                 Befehl nicht.}
            \item[Argument]{:\\Der Kennbuchstabe des verwendeten Formates}
            \end{description}}
\item[1100]{"Uberfl"ussiges Attribut
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Der benutzte Befehl (Prozessor oder Pseudo) darf
                 kein mit einem Punkt angeh"angtes Attribut haben.}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1105]{Attribut darf nur 1 Zeichen lang sein
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Das mit einem Punkt an einen Befehl angeh"angte
                 Attribut mu"s genau ein Zeichen lang sein; weder mehr noch
                 weniger ist erlaubt.}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1110]{Unpassende Operandenzahl
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Die bei einem Befehl (Prozessor oder Pseudo)
                 angegebene Operandenzahl liegt nicht in dem f"ur diesen
                 Befehl erlaubten Bereich.}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1115]{Unpassende Optionenzahl
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Die bei diesem Befehl angegebene Zahl
	         von Optionen liegt nicht in dem f"ur diesen
                 Befehl erlaubten Bereich.}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1120]{nur immediate-Adressierung erlaubt
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Der benutzte Befehl l"a"st nur
                 immediate-Operanden (mit vorangestelltem \#) zu.}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1130]{Unpassende Operandengr"o"se
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Der Operand hat zwar einen f"ur den Befehl
                 zugelassenen Typ, jedoch nicht die richtige L"ange (in
                 Bits).}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1131]{Widersprechende Operandengr"o"sen
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Die angegebenen Operanden haben unterschiedliche
                 L"angen (in Bit).}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1132]{Undefinierte Operandengr"o"se
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Aus Opcode und Operanden l"a"st sich die
		 ...
		 festlegen.}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1135]{Ung"ultiger Operandentyp
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Ein Ausdruck hat einen an dieser Stelle nicht
                 zul"assigen Typ (Integer/Gleitkomma/String).}
            \item[Argument]{:\\Die an dieser Stelle zul"assigen Datentypen}
            \end{description}}
\item[1140]{Zuviele Argumente
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Einem Befehl wurden mehr als die unter AS
                 zul"assigen 20 Parameter "ubergeben.}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1200]{Unbekannter Befehl
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Der benutzte Befehl ist weder ein Pseudobefehl
                 von AS noch ein Befehl des momentan eingestellten
                 Prozessors.}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1300]{Klammerfehler
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Der Formelparser ist auf einen (Teil-)Ausdruck
                 gesto"sen, in dem die Summe "offnender und schlie"sender
                 Klammern nicht "ubereinstimmt.}
            \item[Argument]{:\\der beanstandete (Teil-)Ausdruck}
            \end{description}}
\item[1310]{Division durch 0
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Bei einer Division oder Modulooperation ergab
                 die Auswertung des rechten Teilausdruckes 0.}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1315]{Bereichsunterschreitung
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Der angegebene Integer-Wert unterschreitet
            den zul"assigen Bereich.}
            \item[Argument]{:\\aktueller Wert und zul"assiges Minimum
	                       (manchmal, ich stelle das gerade um...)}
            \end{description}}
\item[1320]{Bereichs"uberschreitung
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Der angegebene Integer-Wert "uberschreitet
            den zul"assigen Bereich.}
            \item[Argument]{:\\aktueller Wert und zul"assiges Maximum
	                       (manchmal,ich stelle das gerade um...)}
            \end{description}}
\item[1325]{Adresse nicht ausgerichtet
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Die angegebene direkte Speicheradresse
            ...
            \item[Argument]{:\\keines}
            \end{description}}
\item[1330]{Distanz zu gro"s
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Der in einem Adre"sausdruck enthaltene
                 Displacement-Wert ist zu gro"s.}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1340]{Kurzadressierung nicht m"oglich
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Die Adresse des Operanden liegt au"serhalb des
                 Speicherbereiches, in dem Kurzadressierung m"oglich ist.}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1350]{Unerlaubter Adressierungsmodus
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Der benutzte Adressierungsmodus existiert
                 generell zwar, ist an dieser Stelle aber nicht erlaubt.}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1351]{Nummer mu"s ausgerichtet sein
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\An dieser Stelle sind nur ausgerichtete
            (d.h z.B. gerade) Adressen erlaubt, da die untersten Bits f"ur
            andere Zwecke verwendet werden oder reserviert sind.}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1355]{Adressierungsmodus im Parallelbetrieb nicht erlaubt
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Die verwendeten Adressierungsmodi
            sind zwar im sequentiellen Modus zul"assig, jedoch nicht
            bei parallelen Instruktionen.}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1360]{Undefinierte Bedingung
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Die benutzte Bedingung f"ur bedingte Spr"unge
                 existiert nicht.}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1370]{Sprungdistanz zu gro"s
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Sprungbefehl und Sprungziel liegen zu weit
                 auseinander, um mit einem Sprung der benutzten L"ange
                 "uberbr"uckt werden zu k"onnen.}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1375]{Sprungdistanz ist ungerade
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Da Befehle nur auf geraden Adressen liegen
            ...
            \item[Argument]{:\\keines}
            \end{description}}
\item[1380]{ung"ultiges Schiebeargument
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\als Argument f"ur die Schiebeamplitude darf nur
                 eine Konstante oder ein Datenregister verwendet werden.
                 (nur 680x0)}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1390]{Nur Bereich 1..8 erlaubt
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Konstanten f"ur Schiebeamplituden oder
                  \tty{ADDQ}-Argumente d"urfen nur im Bereich 1..8 liegen. (nur
                  680x0)}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1400]{Schiebezahl zu gro"s
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\(nicht mehr verwendet)}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1410]{Ung"ultige Registerliste
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Das Registerlisten-Argument von \tty{MOVEM}
	         oder \tty{FMOVEM} hat ein falsches Format. (nur 680x0)}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1420]{Ung"ultiger Modus mit CMP
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Die verwendete Operandenkombination von \tty{CMP}
	    ist nicht erlaubt. (nur 680x0)}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1430]{Ung"ultiger Prozessortyp
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Den mit CPU angeforderten Zielprozessor kennt AS
                 nicht.}
            \item[Argument]{:\\der unbekannte Prozessortyp}
            \end{description}}
\item[1440]{Ung"ultiges Kontrollregister
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Das bei z.B. \tty{MOVEC} benutzte
	         Kontrollregister  kennt der mit CPU gesetzte Prozessor
		 (noch) nicht.}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1445]{Ung"ultiges Register
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Das benutzte Register ist zwar prinzipiell
                 vorhanden, hier aber nicht erlaubt.}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1450]{RESTORE ohne SAVE
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Es wurde ein \tty{RESTORE}-Befehl gefunden, obwohl
                 kein mit \tty{SAVE} gespeicherter Zustand (mehr) auf dem Stapel
                 vorhanden ist.}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1460]{fehlendes RESTORE
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Nach der Assemblierung sind nicht alle
                 \tty{SAVE}-Befehle wieder aufgel"ost worden.}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1465]{unbekannte Makro-Steueranweisung
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Eine beim \tty{MACRO}-Befehl zus"atzlich angegebene
                            Steueranweisung ist AS unbekannt.}
            \item[Argument]{:\\die fragliche Anweisung}
            \end{description}}
\item[1470]{fehlendes ENDIF/ENDCASE
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Nach der Assemblierung sind nicht alle
            ...
            \item[Argument]{:\\keines}
            \end{description}}
\item[1480]{ung"ultiges IF-Konstrukt
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Die Reihenfolge der Befehle in einem \tty{IF}-
	         oder \tty{SWITCH}-Konstrukt stimmt nicht.}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1483]{doppelter Sektionsname
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Es existiert bereits eine Sektion gleichen
                 Namens auf dieser Ebene.}
            \item[Argument]{:\\der doppelte Name}
            \end{description}}
\item[1484]{unbekannte Sektion
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Im momentanen Sichtbarkeitsbereich existiert
                 keine Sektion dieses Namens.}
            \item[Argument]{:\\der unbekannte Name}
            \end{description}}
\item[1485]{fehlendes ENDSECTION
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Nach Ende eines Durchganges sind nicht alle
                 Sektionen wieder geschlossen worden.}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1486]{falsches ENDSECTION
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\die bei \tty{ENDSECTION} angegebene Sektion
	          ist nicht die innerste offene.}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1487]{ENDSECTION ohne SECTION
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Es wurde ein \tty{ENDSECTION}-Befehl gegeben, obwohl
                 gar keine Sektion offen war.}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1488]{nicht aufgel"oste Vorw"artsdeklaration
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\ein mit \tty{FORWARD} oder \tty{PUBLIC}
	         angek"undigtes Symbol wurde nicht in der Sektion definiert.}
            \item[Argument]{:\\der Name des fraglichen Symbols}
            \end{description}}
\item[1489]{widersprechende FORWARD $\leftrightarrow$ PUBLIC-Deklaration
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Ein Symbol wurde sowohl als privat als auch
                 global definiert.}
            \item[Argument]{:\\der Name des Symbols}
            \end{description}}
\item[1490]{falsche Argumentzahl f"ur Funktion
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Die Anzahl der Argumente f"ur eine
                 selbstdefinierte Funktion stimmt nicht mit der geforderten
                 Anzahl "uberein.}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1495]{unaufgel"oste Literale (LTORG fehlt)
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Am Programmende oder beim Umachalten
                 zu einem anderen Zielprozessor blieben noch nicht
                 abgelegte Literale "ubrig.}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1500]{Befehl auf dem ... nicht vorhanden
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Der benutzte Befehl existiert zwar
                 grunds"atzlich, das eingestellte Mitglied der
                 Prozessorfamilie beherrscht ihn aber noch nicht.}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1505]{Adressierungsart auf dem ... nicht vorhanden
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Der benutzte Adressierungsmodus existiert
            ...
            \item[Argument]{:\\keines}
            \end{description}}
\item[1510]{Ung"ultige Bitstelle
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Die angegebene Bitnummer ist nicht erlaubt
	         oder eine Angabe fehlt komplett.}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1520]{nur ON/OFF erlaubt
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Dieser Pseudobefehl darf als Argument nur \tty{ON}
                 oder \tty{OFF} haben.}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1530]{Stack ist leer oder nicht definiert
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Es wurde bei einem \tty{POPV}
                 einen Stack anzusprechen, der entweder nie definiert oder
                 bereits leerger"aumt wurde.}
            \item[Argument]{:\\der Name des fraglichen Stacks}
            \end{description}}
\item[1540]{Nicht genau ein Bit gesetzt
            \begin{description}
	    \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\In einer Bitmaske, die der \tty{BITPOS}-
                 Funktion "ubergeben wurde, war nicht genau ein Bit
                 gesetzt.}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1600]{vorzeitiges Dateiende
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Es wurde mit einem \tty{BINCLUDE}-Befehl versucht,
                 "uber das Ende einer Datei hinauszulesen.}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1700]{ROM-Offset geht nur von 0..63
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Das Konstanten-ROM der 680x0-Koprozessoren hat
                 nur max. 63 Eintr"age.}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1710]{Ung"ultiger Funktionscode
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Als Funktionscodeargument darf nur SFC, DFC, ein
                 Datenregister oder eine Konstante von 0..15 verwendet
                 werden. (nur 680x0-MMU)}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1720]{Ung"ultige Funktionscodemaske
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Als Funktionscodemaske darf nur ein Wert von
                 0..15 verwendet werden. (nur 680x0-MMU)}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1730]{Ung"ultiges MMU-Register
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Die MMU hat kein Register mit dem angegebenen
                 Namen. (nur 680x0-MMU)}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1740]{Level nur von 0..7
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Die Ebene f"ur \tty{PTESTW} und \tty{PTESTR} mu"s eine
                 Konstante von 0..7 sein. (nur 680x0-MMU)}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1750]{ung"ultige Bitmaske
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Die bei den Bit-Feld-Befehlen angegebene
                 Bitmaske hat ein falsches Format. (nur 680x0)}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1760]{ung"ultiges Registerpaar
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Das angegebene Registerpaar ist hier nicht
                 verwendbar oder syntaktisch falsch. (nur 680x0)}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1800]{offene Makrodefinition
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Eine Makrodefinition war am Dateiende nicht
                 zuende.  Vermutlich fehlt ein \tty{ENDM}.}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1805]{EXITM au"serhalb eines Makrorumpfes
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\ \tty{EXITM} bricht die Expansion von
                 Makro-Konstrukten ab.  Dieser Befehl macht nur innerhalb
                 von Makros Sinn und es wurde versucht, ihn au"serhalb
                 aufzurufen.}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1810]{mehr als 10 Makroparameter
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Ein Makro darf h"ochstens 10 Parameter haben.}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1815]{doppelte Makrodefinition
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Ein Makronamne wurde in einer Sektion doppelt
                 vergeben.}
            \item[Argument]{:der doppelt verwendete Name}
            \end{description}}
\item[1820]{Ausdruck mu"s im ersten Pass berechenbar sein
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Der benutzte Befehl beeinflu"st die Codel"ange,
                 daher sind Vorw"artsreferenzen hier nicht erlaubt.}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1830]{zu viele verschachtelte IFs
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\(nicht mehr verwendet)}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1840]{ELSEIF/ENDIF ohne ENDIF
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\es wurde ein \tty{ELSEIF}- oder \tty{ENDIF}-Befehl gefunden,
                 obwohl kein offener \tty{IF}-Befehl vorhanden ist.}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1850]{verschachtelter/rekursiver Makroaufruf
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\(nicht mehr verwendet)}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1860]{unbekannte Funktion
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Die angesprochene Funktion ist weder eingebaut
                 noch nachtr"aglich definiert worden.}
            \item[Argument]{:\\der Funktionsname}
            \end{description}}
\item[1870]{Funktionsargument au"serhalb Definitionsbereich
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Das Argument liegt nicht im Bereich der
                 angesprochenen transzendenten Funktion.}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1880]{Gleitkomma"uberlauf
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Das Argument liegt zwar im Bereich der
                 angesprochenen transzendenten Funktion, das Ergebnis
                 w"are aber nicht mehr darstellbar.}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1890]{ung"ultiges Wertepaar
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Das benutzte P"archen aus Basis und Exponent
                 kann nicht berechnet werden.}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1900]{Befehl darf nicht auf dieser Adresse liegen
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Die Prozessorhardware erlaubt keine
                 Spr"unge von dieser Adresse.}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1905]{ung"ultiges Sprungziel
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Die Prozessorhardware erlaubt keine
            ...
            \item[Argument]{:\\keines}
            \end{description}}
\item[1910]{Sprungziel nicht auf gleicher Seite
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Sprungbefehl und Sprungziel m"ussen bei diesem
                 Befehl auf der gleichen Seite liegen.}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1920]{Code"uberlauf
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Es wurde versucht, mehr als 1024 Bytes Code oder
                 Daten in einer Zeile zu erzeugen.}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1925]{Adre"s"uberlauf
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Der Adre"sraum dieses Prozessors wurde
                 "uberschritten.}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1930]{Konstanten und Platzhalter nicht mischbar
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Anweisungen, die Speicher reservieren und solche,
                 die ihn mit Konstanten belegen, d"urfen nicht in einer
                 Pseudoanweisung gemischt werden.}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1940]{Codeerzeugung nur im Codesegment zul"assig
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Alle Segmente au"ser Code dienen nur der
                 Reservierung von Speicher, in ihnen d"urfen weder Code
                 noch Daten abgelegt werden.\\
                 (nicht mehr benutzt)}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1950]{Paralleles Konstrukt nicht m"oglich
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Entweder sind die beiden Instruktionen
            prinzipiell nicht parallel ausf"uhrbar, oder sie stehen nicht
            unmittelbar untereinander.}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1960]{ung"ultiges Segment
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Das angegebene Segment ist an dieser Stelle
                 nicht anwendbar.}
            \item[Argument]{:\\der benutzte Segmentname}
            \end{description}}
\item[1961]{unbekanntes Segment
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Das angegebene Segment existiert bei
                 diesem Prozessor nicht.}
            \item[Argument]{:\\der benutzte Segmentname}
            \end{description}}
\item[1962]{unbekanntes Segmentregister
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Das angegebene Segmentregister existiert
                 nicht (nur 8086).}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1970]{ung"ultiger String
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Der angegebene String hat ein ung"ultiges
                 Format.}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1980]{ung"ultiger Registername
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Das angegebene Register existiert nicht oder
                 darf hier nicht verwendet werden.}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1985]{ung"ultiges Argument
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Der angegebene Befehl darf nicht mit einem
                 \tty{REP}-Pr"afix versehen werden.}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1990]{keine Indirektion erlaubt
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\in dieser Kombination ist keine indirekte
                 Adressierung erlaubt.}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1995]{nicht im aktuellen Segment erlaubt
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\(nicht mehr verwendet)}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1996]{nicht im Maximum-Modus zul"assig
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Dieses Register ist nur im Minimum-Modus
                              definiert.}
            \item[Argument]{:\\keines}
            \end{description}}
\item[1997]{nicht im Minimum-Modus zul"assig
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Dieses Register ist nur im Maximum-Modus
                              definiert.}
            \item[Argument]{:\\keines}
            \end{description}}
\item[2000]{ung"ultige Pr"afix-Kombination
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Die angegebene Kombination von Pr"afixen
	                      ist nicht zul"assig oder nicht im
			      Maschinenkode darstellbar.}
            \item[Argument]{:\\keines}
            \end{description}}
\item[2010]{Ung"ultige Escape-Sequenz
            \begin{description}
            \item[Fehlertyp]{:\\Fehler}
            \item[Ursache]{:\\Das mit einem Backslash eingeleitete
	                      Sonderzeichen ist nicht definiert.}
            \item[Argument]{:\\keines}
            \end{description}}
\item[10001]{Fehler bein "Offnen der Datei
            \begin{description}
            \item[Fehlertyp]{:\\fatal}
            \item[Ursache]{:\\Beim Versuch, eine Datei zu "offnen, ist ein
                 Fehler aufgetreten.}
            \item[Argument]{:\\Beschreibung des E/A-Fehlers}
            \end{description}}
\item[10002]{Listingschreibfehler
            \begin{description}
            \item[Fehlertyp]{:\\fatal}
            \item[Ursache]{:\\Beim Schreiben des Assemblerlistings ist ein
                 Fehler aufgetreten.}
            \item[Argument]{:\\Beschreibung des E/A-Fehlers}
            \end{description}}
\item[10003]{Dateilesefehler
            \begin{description}
            \item[Fehlertyp]{:\\fatal}
            \item[Ursache]{:\\Beim Lesen aus einer Quelldatei ist ein
                 Fehler aufgetreten.}
            \item[Argument]{:\\Beschreibung des E/A-Fehlers}
            \end{description}}
\item[10004]{Dateischreibfehler
            \begin{description}
            \item[Fehlertyp]{:\\fatal}
            \item[Ursache]{:\\Beim Schreiben von Code- oder Share-Datei
                 ist ein Fehler aufgetreten.}
            \item[Argument]{:\\Beschreibung des E/A-Fehlers}
            \end{description}}
\item[10006]{Speicher"uberlauf
            \begin{description}
            \item[Fehlertyp]{:\\fatal}
            \item[Ursache]{:\\Der verf"ugbare Speicher reicht nicht mehr,
	         alle Datenstrukturen aufzunehmen.  Weichen Sie auf die
		 DPMI- oder OS/2-Version von AS aus.}
            \item[Argument]{:\\keines}
            \end{description}}
\item[10007]{Stapel"uberlauf
            \begin{description}
            \item[Fehlertyp]{:\\fatal}
            \item[Ursache]{:\\Der Programmstapel ist wegen zu komplizierter
...
\item[4]{zu viele offene Dateien\\
         ...
         \tt FILES=\rm -Angabe in der CONFIG.SYS.}
...
Stephan Hruschka, Peter Kliegelh"ofer, Ulf Meinke, Matthias Paul,
...
\item{Version 1.31:
      ...
      \item{zus"atzlicher MCS-51-Prozessortyp 80515.  Die Nummer wird
            ...
            wurde um die dazugeh"origen SFRs erweitert. \bf ACHTUNG! \rm
...
\item{Version 1.39:
      ...
      \item{neben dem MELPS-7700 ist nun auch das ,,Vorbild'' 65816
            vorhanden (p4);}
      \item{Die ST62xx-Pseudoanweisung \tt ROMWIN \rm wurde entfernt und
            mit in den \tt ASSUME\rm -Befehl eingegliedert (p4);}
      \item{unterst"utzt zus"atzlich den 6804 von SGS-Thomson (p4);}
      \item{durch die \tt NOEXPORT\rm -Option in der Makrodefinition
            kann nun f"ur jedes Makro einzeln festgelegt werden, ob es
            in der MAC-Datei erscheinen soll oder nicht (p4);}
      \item{Die Bedeutung von \tt MACEXP \rm f"ur Expansionen von Makros hat
            sich wegen der zus"atzlichen \tt NOEXPAND\rm -Option in der
            Makrodefinition leicht ge"andert (p4);}
      \item{Durch die \tt GLOBAL\rm -Option in der Makrodefinition k"onnen nun
...
\item{Version 1.40:
      ...
      \item{mit der \tt GLOBAL\rm -Anweisung lassen sich Symbolnamen
...
hier eine ziemlich gro"se Prozedur \tt EvalExpression\rm, welche einen
...
die Funktionen \tt EvalIntExpression, EvalFloatExpression \rm und \tt
EvalStringExpression \rm verwendet werden.  "Anderungen zum Einf"ugen neuer
...
definiert sind, z.B. \tt EQU \rm und \tt ORG\rm.  Hier findet sich auch der
\tt CPU\rm -Befehl, mit dem zwischen den einzelnen Prozessoren hin-und
hergeschaltet wird.

\subsubsection{CODEPSEU.PAS}

Hier finden sich Pseudobefehle, die von mehreren Codegeneratoren verwendet
werden.  Dies ist einmal die Intel-Gruppe mit der \tt DB..DT-\rm Gruppe,
...
\tt BIND, P2HEX und P2BIN \rm verwendet.
...
man eine korrekt f"ur alle Zeichen arbeitende \tt UpCase-\rm Funktion, die
...
Vorteil, da"s in den Standard-Modulen von AS nur noch an \bf EINER \rm Stelle
...
einer gewissen Einheitlichkeit wegen den Namen \tt CODExxxx \rm tragen,
wobei \tt xxxx \rm etwas mit dem Prozessornamen zu tun haben sollte.  Den
Kopf mit den Compilerschaltern sowie den \tt Uses\rm -Anweisungen "ubernimmt
...
ein Aufruf der Funktion \tt AddCPU \rm erfolgt:
\begin{verbatim}
   CPUxxxx:=AddCPU('XXXX',SwitchTo_xxxx);
\end{verbatim}
\tt 'XXXX' \rm ist dabei der f"ur den Prozessor festgelegte Name, der sp"ater
im Assemblerprogramm verwendet werden mu"s, um AS auf diesen Zielprozessor
umzuschalten.  \tt SwitchTo\_xxxx \rm (im folgenden kurz als ,,Umschalter''
...
\tt AddCPU \rm eine Zahlenwert, der als interne ,,Kennung'' f"ur diesen Prozessor
fungiert.  In der globalen Variablen \tt MomCPU \rm wird st"andig die Kennung
des momentan gesetzten Zielprozessors mitgef"uhrt.  Der von \tt AddCPU \rm
gelieferte Wert sollte in einer privaten Variable des Typs \tt CPUVar \rm (hier
\tt CPUxxxx \rm genannt) abgelegt werden.  Falls ein Codegeneratormodul
verschiedene Prozessoren (z.B. einer Familie) verwaltet, kann es so
durch Vergleich von \tt MomCPU \rm gegen diese Werte feststellen, welche
...
\item{\tt IntelConsts\rm : Dieses Flag mu"s auf TRUE gesetzt werden, falls
      Integer-Konstanten nicht in Motorola-, sondern Intelsyntax mit
      hintangestelltem Buchstaben geschrieben werden sollen.}
\item{\tt PCSymbol\rm : Diese Variable enth"alt den String, mit dem aus dem
      ...
      Dollarzeichen.}
\item{\tt HeaderID\rm : Dieses Byte enth"alt die Kennung, mit der in der Codedatei
...
\item{\tt NOPCode\rm : In bestimmten Situationen kann es sein, da"s AS unbenutzte
      Bereiche im Code mit NOPs auff"ullen mu"s.  Diese Variable beinhaltet
      den dazu erforderlichen Code.}
\item{\tt SetIsOccupied\rm : Einige Prozessoren verwenden \tt SET \rm als
      Maschinenbefehl.  Ist diese Variable gesetzt, so gibt AS \tt SET\rm
      -Befehle an den Codegenerator weiter und verwendet stattdessen \tt EVAL\rm .}
\item{\tt ValidSegs\rm : Nicht alle Prozessoren definieren alle von AS
      ...
      Modul \tt ASMDEF \rm nachgelesen werden (\tty{Seg}.....-Konstanten).}
\item{\tt SegInits\rm : Dieses Feld speichert die initialen (ohne \tty{ORG}-Befehl)
...
\item{\tt Grans\rm : Hiermit kann f"ur jedes Segment die Gr"o"se des kleinsten
      ...
      Rahmen.}
\item{\tt ListGrans\rm : Hiermit kann wieder f"ur alle Segmente getrennt
      ...
      auf 2 gesetzt ist.}
\item{\tt TurnWords\rm : Falls der Prozessor ein Big-Endian-Prozessor sein
      sollte und eines der Elemente von \tt ListGrans \rm ungleich eins ist,
...
Wert hat, sondern besetzen Sie \bf ALLE \rm Felder neu!!
\par
...
\begin{itemize}
\item{\tt MakeCode \rm : Diese Routine wird nach der Zerlegung einer Zeile
      in Mnemonic und Parameter aufgerufen.  Das Mnemonic liegt in der
      Variablen \tty{OpPart}, die Parameter in dem Feld \tt ArgStr\rm .
      Die Zahl der Parameter kann aus der Variablen \tt ArgCnt \rm ausgelesen
      werden.  Das bin"are Ergebnis mu"s in dem Byte-Feld \tt BAsmCode \rm
      abgelegt werden, dessen L"ange in der Variablen \tt CodeLen\rm .  Falls
      der Prozessor wortorientiert wie der 68000 oder viele Signalprozessoren
      ist, kann Feld auch wortweise als \tt WAsmCode \rm adressiert werden.
      F"ur ganz extreme F"alle gibt es auch noch \tt DAsmCode\rm ... Die
      Codel"ange wird ebenfalls in solchen Einheiten angegeben.}
\item{\tt ChkPC \rm : Obwohl AS die Programmz"ahler intern durchg"angig mit
      ...
      findet sich z.B. im Modul \tt Code51\rm .  Falls alles in Ordnung ist,
      mu"s die Funktion TRUE zur"uckliefern, ansonsten FALSE.
      \bf VORSICHT! \rm  Da Turbo-Pascal momentan noch keine vorzeichenlosen
      ...
      unbedingt notwendig!}
\item{\tt IsDef \rm : Bestimmte Prozessoren kennen neben \tt EQU \rm noch weitere
      Pseudobefehle, bei denen ein in der ersten Spalte stehender
      Symbolname kein Label darstellt, z.B. \tt BIT \rm beim 8051.  Diese
...
indem er in der Initialisierung der Unit (bei den \tt AddCPU\rm -Befehlen)
einen Aufruf der Prozedur \tt AddCopyright \rm einf"ugt, in der folgenden
Art:
\begin{verbatim}
...
\tt ASMSUB\rm und \tty{ASMPARS}.  Ich kann hier nur einige generelle
Hinweise geben:
\begin{itemize}
...
\item{Die Routine \tt WrError \rm definiert eine Vielzahl von m"oglichen
...
Damit \tt PLIST \rm Codedateien weiterhin vollst"andig anzeigen kann, mu"s das
Feld mit den vorhandenen Code-Headern erweitert werden.
\par
Sollte ein Segment eine andere Granularit"at als 1 Byte/Adresse
aufweisen (s. die Variable \tt Grans \rm oben), so mu"s die Funktion
\tt Granularity \rm in \tt TOOLS.PAS \rm erweitert werden.  Damit \tt P2HEX \rm
...
Intel-, Tektronix- und MOS-HEX (16-Bit-Adressen).  Die \tt CASE\rm -Abfrage in
\tt ProcessFile \rm mu"s nur entsprechend erweitert werden.
