	...
	static PChainRec DecodeChain(char *Asc)
{
   ...
   Rec->Next=NULL; Rec->RegCnt=0; Rec->DispAcc=0; Rec->HasDisp=false;
   ...
   while ((*Asc!='\0') && (! ErrFlag))
    {
     ...
     strcpy(SReg,Part); p=QuotPos(SReg,'*'); if (p!=NULL) *p='\0';
   ...
   if (*Asc=='@')
    {
     ...
     /* komplex: dann chained iterieren */
     ...
     else
      {
       ...
       z=0; while ((z<RunChain->RegCnt) && (RunChain->Scales[z]!=0)) z++;
	...
	{
         RunChain=RootChain; PrevChain=NULL;
             ...
             if (RunChain->DSize==DispSizeNone)
              {
               ...
               else if ((RunChain->DispAcc>=-0x8000-MinReserve) &&
         ...
         if ((RunChain->RegCnt==0) && (! RunChain->HasDisp))
          {
           ...
           if (PrevChain==NULL) RootChain=NULL; else PrevChain->Next=NULL;
           free(RunChain);
          }
	}
   ...
   while (*Asc!='\0')
    {
     ...
     if ((p1!=NULL) && (p1<p2)) p=p1; else p=p2;
     if (p==NULL)
      {
             ...
             switch (FormatCode)
              {
               ...
               case 2:
                Make_E(0x4000+(z << 10),! Odd(z)); break;
               case 3:
                if ((AdrType[1]!=ModReg) || (AdrType[2]!=ModReg)) WrError(1350);
                else if ((OpSize[1]!=2) || (OpSize[2]!=2)) WrError(1130);
                else
