            ...
            switch (ActFormat) {
               ...
               case TiDSK:
                  if (!DSKOccured) {
                     DSKOccured = true;
                     errno = 0;
                     fprintf(TargFile, "%s%s\n", DSKHeaderLine, TargName);
                     ChkIO(TargName);
                  }
                  break;
               default:
            ...
            /* Start der Datenzeile */
               ...
               switch (ActFormat) {
                  case MotoS:
                     ...
                     if (MotRecType >= 2) {
                        errno = 0;
                        fprintf(TargFile, "%s", HexByte((ErgStart >> 24) & 0xff));
                        ChkIO(TargName);
                        ChkSum += ((ErgStart >> 24) & 0xff);
                     }
                     if (MotRecType >= 1) {
                        errno = 0;
                        fprintf(TargFile, "%s", HexByte((ErgStart >> 16) & 0xff));
                        ChkIO(TargName);
                        ChkSum += ((ErgStart >> 16) & 0xff);
                     }
                     errno = 0;
                     fprintf(TargFile, "%s", HexWord(ErgStart & 0xffff));
                     ChkIO(TargName);
                     ChkSum += Hi(ErgStart) + Lo(ErgStart);
                     break;
                  case MOSHex:
                     errno = 0;
                     fprintf(TargFile, ";%s%s", HexByte(TransLen), HexWord(ErgStart && 0xffff));
                     ChkIO(TargName);
                  ...
                  case IntHex32:
                     ...
                     if (MultiMode == 0) {
                        errno = 0;
                        fprintf(TargFile, "%s", HexByte(TransLen));
                        ChkIO(TargName);
                        errno = 0;
                        fprintf(TargFile, "%s", HexWord((ErgStart - IntOffset) * Gran));
                        ChkIO(TargName);
                        ChkSum += TransLen;
                        ...
                        errno = 0;
                        fprintf(TargFile, "%s", HexByte(TransLen / Gran));
                        ChkIO(TargName);
                        errno = 0;
                        fprintf(TargFile, "%s", HexWord(ErgStart - IntOffset));
                        ChkIO(TargName);
                  ...
                  case TekHex:
                     errno = 0;
                     fprintf(TargFile, "/%s%s%s", HexWord(ErgStart), HexByte(TransLen), HexByte(Lo(ErgStart) + Hi(ErgStart) + TransLen));
                     ChkIO(TargName);
                     ChkSum = 0;
                     break;
                  case TiDSK:
                     errno = 0;
                     fprintf(TargFile, "9%s", HexWord( /*Gran* */ ErgStart));
                     ChkIO(TargName);
                     ChkSum = 0;
                     break;
                  default:
               ...
               if (ActFormat == TiDSK) {
                  ...
                  for (z = 0; z < (LongInt) TransLen; z++)
                     if ((MultiMode < 2) || (z % Gran == MultiMode - 2)) {
                        errno = 0;
                        fprintf(TargFile, "%s", HexByte(Buffer[z]));
                        ChkIO(TargName);
                        ChkSum += Buffer[z];
                        SumLen++;
                     }
...
int main(int argc, char **argv) {
   ...
   *TargName = '\0';
   ...
   if ((MotoOccured) && (!SepMoto)) {
      errno = 0;
      fprintf(TargFile, "S%c%s", '9' - MaxMoto, HexByte(3 + MaxMoto));
      ChkIO(TargName);
      ChkSum = 3 + MaxMoto;
      if (!EntryAdrPresent) EntryAdr = 0;
      if (MaxMoto >= 2) {
         errno = 0;
         fprintf(TargFile, HexByte((EntryAdr >> 24) & 0xff));
         ChkIO(TargName);
         ChkSum += (EntryAdr >> 24) & 0xff;
      }
      if (MaxMoto >= 1) {
         errno = 0;
         fprintf(TargFile, HexByte((EntryAdr >> 16) & 0xff));
         ChkIO(TargName);
         ChkSum += (EntryAdr >> 16) & 0xff;
      }
      errno = 0;
      fprintf(TargFile, "%s", HexWord(EntryAdr & 0xffff));
      ChkIO(TargName);
      ChkSum += (EntryAdr >> 8) & 0xff;
      ChkSum += EntryAdr & 0xff;
      errno = 0;
      fprintf(TargFile, "%s\n", HexByte(0xff - (ChkSum & 0xff)));
      ChkIO(TargName);
   }
   ...
   if (IntelOccured) {
      if (EntryAdrPresent) {
         if (MaxIntel == 2) {
            errno = 0;
            fprintf(TargFile, ":04000003");
            ChkIO(TargName);
            ChkSum = 4 + 3;
            errno = 0;
            fprintf(TargFile, "%s", HexLong(EntryAdr));
            ChkIO(TargName);
            ChkSum += ((EntryAdr >> 24) & 0xff) + ((EntryAdr >> 16) & 0xff) + ((EntryAdr >> 8) & 0xff) + (EntryAdr & 0xff);
         } else if (MaxIntel == 1) {
            errno = 0;
            fprintf(TargFile, ":04000003");
            ChkIO(TargName);
            ...
            Ofs = EntryAdr & 0x000f;
            errno = 0;
            fprintf(TargFile, "%s%s", HexWord(Seg), HexWord(Ofs));
            ChkIO(TargName);
            ChkSum += Lo(Seg) + Hi(Seg) + Ofs;
         } else {
            errno = 0;
            fprintf(TargFile, ":02000003%s", HexWord(EntryAdr & 0xffff));
            ChkIO(TargName);
            ChkSum = 2 + 3 + Lo(EntryAdr) + Hi(EntryAdr);
         }
         errno = 0;
         fprintf(TargFile, "%s\n", HexByte(0x100 - ChkSum));
         ChkIO(TargName);
      }
      errno = 0;
      switch (IntelMode) {
         case 0:
            fprintf(TargFile, ":00000001FF\n");
            break;
         case 1:
