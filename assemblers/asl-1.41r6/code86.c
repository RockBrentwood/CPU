...
BEGIN
   ...
   ALU2Orders[InstrZ++]="ADD"; ALU2Orders[InstrZ++]="OR";
   ALU2Orders[InstrZ++]="ADC"; ALU2Orders[InstrZ++]="SBB";
   ALU2Orders[InstrZ++]="AND"; ALU2Orders[InstrZ++]="SUB";
   ALU2Orders[InstrZ++]="XOR"; ALU2Orders[InstrZ++]="CMP";
...
static ONOFFRec ONOFF86s[ONOFF86Count]=
	...
	AdrWord=EvalIntExpression(ArgStr[1],Int16,&OK);
	 ...
	 BEGIN
	  ChkSpace(SegCode);
	   ...
	   BEGIN
	    PutCode(RelOrders[z].Code); BAsmCode[CodeLen++]=Lo(AdrWord);
	   END
	 END
       END
      return;
         ...
         switch (AdrType)
  	    ...
  	    if ((AdrByte==2) OR ((AdrByte==0) AND (AbleToSign(AdrWord-EProgCounter()-2))))
  	     ...
  	     BEGIN
  	      ...
  	      ChkSpace(SegCode);
  	      BAsmCode[0]=0xe8; if (Memo("JMP")) BAsmCode[CodeLen]++;
  	      BAsmCode[1]=Lo(AdrWord);
    ...
    BEGIN
     if (ArgCnt!=1) WrError(1110);
      ...
      BEGIN
       ...
       MinOneIs0();
       BAsmCode[CodeLen]=0xf6+OpSize;
       BAsmCode[CodeLen+1]=0x10+(Ord(Memo("NEG")) << 3);
       switch (AdrType)
        BEGIN
         case TypeReg8:
         case TypeReg16:
