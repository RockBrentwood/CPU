...
static CPUVar CPU75402, CPU75004, CPU75006, CPU75008, CPU75268, CPU75304, CPU75306, CPU75308, CPU75312, CPU75316, CPU75328, CPU75104, CPU75106, CPU75108, CPU75112, CPU75116, CPU75206, CPU75208, CPU75212, CPU75216, CPU75512, CPU75516;
static Word ROMEnd;
...
static void ChkDataPage(Word Adr) {
   switch (MBEValue) {
      case 0:
         if ((Adr > 0x7f) && (Adr < 0xf80)) WrError(110);
         break;
      case 1:
         if (Hi(Adr) != MBSValue) WrError(110);
         break;
   }
}
         ...
         switch (AdrMode) {
            case ModReg4:
               ...
               switch (AdrMode) {
                  case ModReg4:
                     if (HReg == 0) {
                        PutCode(0x7899 + (((Word) AdrPart) << 8));
                        CheckCPU(CPU75004);
                     } else if (AdrPart == 0) {
                        PutCode(0x7099 + (((Word) HReg) << 8));
                        CheckCPU(CPU75004);
                     } else WrError(1350);
                     break;
                  case ModInd:
            ...
            case ModReg8:
               ...
               switch (AdrMode) {
                  case ModReg8:
                     if (HReg == 0) {
                        PutCode(0x58aa + (((Word) AdrPart) << 8));
                        CheckCPU(CPU75004);
                     } else if (AdrPart == 0) {
                        PutCode(0x50aa + (((Word) HReg) << 8));
                        CheckCPU(CPU75004);
                     } else WrError(1350);
                     break;
                  case ModAbs:
         ...
         switch (AdrMode) {
            ...
            case ModReg8:
               ...
               switch (AdrMode) {
                  case ModReg8:
                     if (HReg == 0) {
                        PutCode(0x40aa + (((Word) AdrPart) << 8));
                        CheckCPU(CPU75004);
                     } else if (AdrPart == 0) {
                        PutCode(0x40aa + (((Word) HReg) << 8));
                        CheckCPU(CPU75004);
                     } else WrError(1350);
                     break;
                  case ModAbs:
         ...
         switch (AdrMode) {
            case ModReg4:
               if (AdrPart != 0) WrError(1350);
                  ...
                  switch (AdrMode) {
                     case ModImm:
                        PutCode(0x60 + AdrPart);
                        break;
                     case ModInd:
                        if (AdrPart == 1) PutCode(0xd2);
                        else WrError(1350);
                        break;
                  }
               }
                  ...
                  if (AdrPart != 0) WrError(1350);
                     ...
                     switch (AdrMode) {
                        case ModImm:
                           ...
                           break;
                        case ModInd:
                           if (AdrPart == 1) PutCode(0x80 + ((z + 1) << 4));
                           else WrError(1350);
                           break;
                     }
                  }
...
static void SwitchTo_75K0(void) {
   ...
   MakeCode = MakeCode_75K0;
   ChkPC = ChkPC_75K0;
   IsDef = IsDef_75K0;
   SwitchFrom = SwitchFrom_75K0;
   InitFields();
}
