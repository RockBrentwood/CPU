...
// Constant / for increment bit in indirect operand.
#define	AUTOINC		1
#define	NOINC		0
long labelloc;
...
line: KOC_CPU STRING {
   if (!cpumatch($2)) fraerror("unknown cpu type, 80196 assumed"), cpuselect = CPU196;
};
...
genline: KOC_opcode expr {
   genlocrec(currseg, labelloc);
   pevalexpr(1, $2);
   locctr += geninstr(findgen($1, ST_DIR1, cpuselect + (evalr[1].seg == SSG_ABS && evalr[1].value >= 0 && evalr[1].value <= 0xff? DIRECT: EXTENDED)));
};
genline: KOC_opcode expr ',' expr {
   genlocrec(currseg, labelloc);
   pevalexpr(1, $2);
   pevalexpr(2, $4);
   locctr += geninstr(findgen($1, ST_DIR2, cpuselect + (evalr[2].seg == SSG_ABS && evalr[2].value >= 0 && evalr[2].value <= 0xff? DIRECT: EXTENDED)));
};
genline: KOC_opcode expr ',' expr ',' expr {
   ...
   pevalexpr(2, $4);
   pevalexpr(3, $6);
   locctr += geninstr(findgen($1, ST_DIR3, cpuselect + (evalr[3].seg == SSG_ABS && evalr[3].value >= 0 && evalr[3].value <= 0xff? DIRECT: EXTENDED)));
};
genline: KOC_opcode '#' expr {
   genlocrec(currseg, labelloc);
   pevalexpr(1, $3);
...
genline: KOC_opcode expr '[' expr ']' {
   ...
   pevalexpr(1, $2);
   pevalexpr(2, $4);
   locctr += geninstr(findgen($1, ST_INX1, cpuselect + (evalr[1].seg == SSG_ABS && evalr[1].value >= -0x80 && evalr[1].value <= 0x7f? DIRECT: EXTENDED)));
};
genline: KOC_opcode expr ',' expr '[' expr ']' {
   genlocrec(currseg, labelloc);
   pevalexpr(1, $2);
   pevalexpr(2, $4);
   pevalexpr(3, $6);
   locctr += geninstr(findgen($1, ST_INX2, cpuselect + (evalr[2].seg == SSG_ABS && evalr[2].value >= -0x80 && evalr[2].value <= 0x7f? DIRECT: EXTENDED)));
};
genline: KOC_opcode expr ',' expr ',' expr '[' expr ']' {
   ...
   pevalexpr(3, $6);
   pevalexpr(4, $8);
   locctr += geninstr(findgen($1, ST_INX3, cpuselect + (evalr[3].seg == SSG_ABS && evalr[3].value >= -0x80 && evalr[3].value <= 0x7f? DIRECT: EXTENDED)));
};
...
struct opsym optab[] = {
   {"invalid", KOC_opcode, 2, 0}, {"ADD", KOC_opcode, 8, 2}, {"ADDB", KOC_opcode, 8, 10}, {"ADDC", KOC_opcode, 4, 18},
   {"ADDCB", KOC_opcode, 4, 22}, {"AND", KOC_opcode, 8, 26}, {"ANDB", KOC_opcode, 8, 34}, {"BMOV", KOC_opcode, 1, 42},
   {"BR", KOC_opcode, 1, 43}, {"BYTE", KOC_BDEF, 0, 0}, {"CHARDEF", KOC_CHDEF, 0, 0}, {"CHARSET", KOC_CHSET, 0, 0},
   {"CHARUSE", KOC_CHUSE, 0, 0}, {"CHD", KOC_CHDEF, 0, 0}, {"CLR", KOC_opcode, 1, 44}, {"CLRB", KOC_opcode, 1, 45},
   {"CLRC", KOC_opcode, 1, 46}, {"CLRVT", KOC_opcode, 1, 47}, {"CMP", KOC_opcode, 4, 48}, {"CMPB", KOC_opcode, 4, 52},
   {"CMPL", KOC_opcode, 1, 56}, {"CPU", KOC_CPU, 0, 0}, {"DCB", KOC_BDEF, 0, 0}, {"DCL", KOC_ALDEF, 0, 0},
   {"DCW", KOC_AWDEF, 0, 0}, {"DEC", KOC_opcode, 1, 57}, {"DECB", KOC_opcode, 1, 58}, {"DI", KOC_opcode, 1, 59},
   {"DIV", KOC_opcode, 4, 60}, {"DIVB", KOC_opcode, 4, 64}, {"DIVU", KOC_opcode, 4, 68}, {"DIVUB", KOC_opcode, 4, 72},
   {"DJNZ", KOC_opcode, 1, 76}, {"DJNZW", KOC_opcode, 1, 77}, {"DSB", KOC_RESM, 0, 0}, {"DSL", KOC_ALRESM, 0, 0},
   {"DSW", KOC_AWRESM, 0, 0}, {"EI", KOC_opcode, 1, 78}, {"ELSE", KOC_ELSE, 0, 0}, {"END", KOC_END, 0, 0},
   {"ENDI", KOC_ENDI, 0, 0}, {"EQU", KOC_EQU, 0, 0}, {"EXT", KOC_opcode, 1, 79}, {"EXTB", KOC_opcode, 1, 80},
   {"FCB", KOC_BDEF, 0, 0}, {"FCC", KOC_SDEF, 0, 0}, {"FDB", KOC_AWDEF, 0, 0}, {"IDLPD", KOC_opcode, 1, 81},
   {"IF", KOC_IF, 0, 0}, {"INC", KOC_opcode, 1, 82}, {"INCB", KOC_opcode, 1, 83}, {"INCL", KOC_INCLUDE, 0, 0},
   {"INCLUDE", KOC_INCLUDE, 0, 0}, {"JBC", KOC_opcode, 1, 84}, {"JBS", KOC_opcode, 1, 85}, {"JC", KOC_opcode, 1, 86},
   {"JE", KOC_opcode, 1, 87}, {"JGE", KOC_opcode, 1, 88}, {"JGT", KOC_opcode, 1, 89}, {"JH", KOC_opcode, 1, 90},
   {"JLE", KOC_opcode, 1, 91}, {"JLT", KOC_opcode, 1, 92}, {"JNC", KOC_opcode, 1, 93}, {"JNE", KOC_opcode, 1, 94},
   {"JNH", KOC_opcode, 1, 95}, {"JNST", KOC_opcode, 1, 96}, {"JNV", KOC_opcode, 1, 97}, {"JNVT", KOC_opcode, 1, 98},
   {"JST", KOC_opcode, 1, 99}, {"JV", KOC_opcode, 1, 100}, {"JVT", KOC_opcode, 1, 101}, {"LCALL", KOC_opcode, 1, 102},
   {"LD", KOC_opcode, 4, 103}, {"LDB", KOC_opcode, 4, 107}, {"LDBSE", KOC_opcode, 4, 111}, {"LDBZE", KOC_opcode, 4, 115},
   {"LJMP", KOC_opcode, 1, 119}, {"LONG", KOC_ALDEF, 0, 0}, {"MUL", KOC_opcode, 8, 120}, {"MULB", KOC_opcode, 8, 128},
   {"MULU", KOC_opcode, 8, 136}, {"MULUB", KOC_opcode, 8, 144}, {"NEG", KOC_opcode, 1, 152}, {"NEGB", KOC_opcode, 1, 153},
   {"NOP", KOC_opcode, 1, 154}, {"NORML", KOC_opcode, 1, 155}, {"NOT", KOC_opcode, 1, 156}, {"NOTB", KOC_opcode, 1, 157},
   {"OR", KOC_opcode, 4, 158}, {"ORB", KOC_opcode, 4, 162}, {"ORG", KOC_ORG, 0, 0}, {"POP", KOC_opcode, 3, 166},
   {"POPA", KOC_opcode, 1, 169}, {"POPF", KOC_opcode, 1, 170}, {"PUSH", KOC_opcode, 4, 171}, {"PUSHA", KOC_opcode, 1, 175},
   {"PUSHF", KOC_opcode, 1, 176}, {"RESERVE", KOC_RESM, 0, 0}, {"RET", KOC_opcode, 1, 177}, {"RMB", KOC_RESM, 0, 0},
   {"RST", KOC_opcode, 1, 178}, {"SCALL", KOC_opcode, 1, 179}, {"SET", KOC_SET, 0, 0}, {"SETC", KOC_opcode, 1, 180},
   {"SHL", KOC_opcode, 2, 181}, {"SHLB", KOC_opcode, 2, 183}, {"SHLL", KOC_opcode, 2, 185}, {"SHR", KOC_opcode, 2, 187},
   {"SHRA", KOC_opcode, 2, 189}, {"SHRAB", KOC_opcode, 2, 191}, {"SHRAL", KOC_opcode, 2, 193}, {"SHRB", KOC_opcode, 2, 195},
   {"SHRL", KOC_opcode, 2, 197}, {"SJMP", KOC_opcode, 1, 199}, {"SKIP", KOC_opcode, 1, 200}, {"ST", KOC_opcode, 3, 201},
   {"STB", KOC_opcode, 3, 204}, {"STRING", KOC_SDEF, 0, 0}, {"SUB", KOC_opcode, 8, 207}, {"SUBB", KOC_opcode, 8, 215},
   {"SUBC", KOC_opcode, 4, 223}, {"SUBCB", KOC_opcode, 4, 227}, {"WORD", KOC_AWDEF, 0, 0}, {"XOR", KOC_opcode, 4, 231},
