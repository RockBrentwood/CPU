...
typedef bool (*TLayoutFunc)(
#ifdef __PROTOS__
                               char *Asc, Word *Cnt, bool Turn
#endif
                                                                 );
	...
	static bool DecodeIntelPseudo_ValidSymChar(char ch)
{
   return (((ch>='A') && (ch<='Z')) || ((ch>='0') && (ch<='9')) || (ch=='_') || (ch=='.'));
	...
	static bool DecodeIntelPseudo_LayoutMult(char *Asc_O, Word *Cnt,
...
static ONOFFRec ONOFFMoto16s[ONOFFMoto16Count]=
   ...
   if (Memo("DS"))
       ...
       if ((ValOK) && (! FirstPassUnknown))
	{
	 ...
	 switch (OpSize)
          {
	   case 0: WSize=1; if (((HVal&1)==1) && (DoPadding)) HVal++; break;
