...
#define RegGBR (-3)
   ...
   if (*Asc=='#')
       ...
       else if ((Mask & MModPCRel)!=0)
	{
	 ...
	 /* schon vorhanden ? */
	 Lauf=FirstLiteral; p=0; OK=false; Last=NULL; Found=false;
	 ...
	 if (! Found)
	  {
	   ...
	   Lauf->Next=NULL; Lauf->PassNo=1; Lauf->DefSection=MomSectionHandle;
           do
            {
           ...
           while (LDef);
           if (Last==NULL) FirstLiteral=Lauf; else Last->Next=Lauf;
	  }
	 ...
	 if (OK)
	  {
	   ...
	   if (DispAcc<0)
	    ...
	    {
	     AdrMode=ModPCRel; AdrPart=DispAcc; OpSize=NIs32+1;
	    }
	  }
	}
       else WrError(1350);
   ...
   if (Memo("LTORG"))
       ...
       else
	{
	 LTORG_16(); LTORG_32();
       ...
       Lauf=FirstLiteral; Last=NULL;
       while (Lauf!=NULL)
	{
	 if ((Lauf->DefSection==MomSectionHandle) && (Lauf->PassNo<0))
	  {
	   Tmp=Lauf->Next;
	   if (Last==NULL) FirstLiteral=Tmp; else Last->Next=Tmp;
	   free(Lauf); Lauf=Tmp;
	  }
	 else
	  {
...
{
   SaveInitProc();
   FirstLiteral=NULL; ForwardCount=0;
}
